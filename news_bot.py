...
def score_item(title, all_titles, cache=None):
    """СТАРИЯТ скор по ключови думи (0..6). НИКОЙ НЕ ГО ВИКА.

    🔴 ТУК ПИШЕШЕ «пази се непокътнат, защото news_showcase.py го вика».
    Измерено на 05.09.2026: score_item се среща ЕДИН път в целия проект —
    този ред. news_showcase.py не го вика и дори не внася news_bot (внася
    само sys). Тоест твърдението беше невярно.

    Това е по-лошо от мъртъв код: мъртвият код мълчи, а такъв коментар
    ГОВОРИ и казва на следващия «не пипай, друг файл разчита». Лъжливото
    твърдение беше единственото, което пазеше функцията.

    Оставена е нарочно: живият подбор минава през rank_story и тази е
    резервата, ако подборът се смени. Но нека изборът да се прави върху
    вярно описание.
    """
    t = title.lower()

...
def _bez_samoproverkata(src):
    ...
    if i < 0:
        return src
    # краят на функцията: първото следващо „def " или „if __name__" в колона 0
    kraj = len(src)
    for marker in (chr(10) + "def ", chr(10) + "if __name__"):
        j = src.find(marker, i + len(igla))
        if j >= 0:
            kraj = min(kraj, j)
    return src[:i] + src[kraj:]

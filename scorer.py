...
def bez_red(n, ime=""):
    ...
    edin = (n == 1)
    return ["\U0001f6ab " + str(n) + (" мач" if edin else " мача")
            + ((" на " + ime) if ime else "") + " без резултат",
            "   източникът не " + ("го" if edin else "ги") + " дава — не "
            + ("го" if edin else "ги") + " броим"]
...
def results_text(now, rows, total_all, hit_all, bez=None, vsichki=None):
    ...
    out = ["\U0001f4ca <b>ОБЗОР</b> · " + chas_bg(now), "",
           ("<b>" + str(_poz) + " от " + str(len(_vid)) + "</b> · "
            + str(_gres) + (" сгрешена" if _gres == 1 else " сгрешени")
            + " · <b>" + ("%.0f" % _pct) + "%</b>")]
...
def obshto_dosega_text(now, rows):
    ...
    ПРЕНАПИСАН 25.08.2026 — текстът, не сметката. Три неща:
      1) Знаменателят пръв: „От 500 пуснати 389 имат резултат", а не колона, в
         която 500 стои горе, а 389 се появява четири реда по-долу.
      2) Неотсъдимите се обясняват с общия ред (bez_red), не с „Без присъда".
      3) 💰 „Струва ли си" отпада. В него стояха „средна цена 1.76" и
         „доходност при равен залог −15.5%" — среден коефициент под друга дума
         и възвръщаемост на залог, в канал, който твърди, че не е за залози.
         Сметките dohodnost() и clv_text() НЕ СА ПИПАНИ и си стоят във файла за
         диагностика; спира само печатането им в стаята. Това е ЗАГУБА на
         число, което собственикът може да е ползвал — казвам го, не го крия.
    """
    rows = list(rows or [])
    ...
    out = ["\U0001f9fe <b>ДОСЕГА ОБЩО</b> · всичко, пуснато в 🤖 БОТА ПРЕДРИЧА",
           ""]
    out.append("От <b>" + str(pusnati) + "</b> пуснати <b>" + str(len(ots))
               + "</b> имат резултат.")
    if ots:
        out.append("✅ познати <b>" + str(poz) + "</b> · ❌ загубени <b>"
                   + str(zag) + "</b> · <b>"
                   + ("%.0f" % (100.0 * poz / len(ots))) + "%</b>")
    else:
        out.append("Още нищо не е отсъдено.")
    # Честността е в знаменателя: колко от пуснатото ОЩЕ не е претеглено.
    if chakat:
        out.append("⏳ " + str(chakat)
                   + (" чака мача си" if chakat == 1 else " чакат мачовете си"))
    out += bez_red(bez_prisada)
    if dni:
        out.append("\U0001f4c5 от " + den_kratko(dni[0]) + " до "
                   + den_kratko(dni[-1]) + " · " + str(len(dni))
    ...
    #
    # 25.08.2026: този блок НЕ Е ПИПАН — включително правилото „под пет
    # отсъдени процент не се дава". Тук е ЕДИНСТВЕНОТО място, където лайфтаймът
    # по спорт още живее: обзорът вече не го носи и ако се пипне и тук, числото
    # изчезва отвсякъде.
    ...
    # 📊 СРЕЩУ ПАЗАРА (18.08.2026). Появява се САМО когато има достатъчно
    # двойки „наша вероятност + пазарна". Дотогава мълчи — празен раздел с
    # обещание е по-лош от липсващ.
    #
    # Смисълът: 70% познати звучи добре, но не казва имаме ли РЪБ. Ръб има
    ...
    if len(pz) >= PAZAR_MIN_KARTA:
        ...
        if len(smeli) >= 10:
            sbd = 100.0 * sum(1 for r in smeli if r["hit"]) / len(smeli)
            paz = 100.0 * sum(float(r["pazar_p"]) for r in smeli) / len(smeli)
            out += ["", "📊 <b>Когато не сме съгласни с пазара</b>",
                    ("  В <b>" + str(len(smeli)) + "</b> мача сме дали по-висок"
                     " шанс от пазара."),
...
def den_finish_text(now, rows, den, mezhdinna=False):
    ...
    out.append("От <b>" + str(len(dnes)) + "</b> пуснати днес <b>"
               + str(len(otsadeni)) + "</b> имат резултат.")
    if otsadeni:
        out.append("✅ познахме <b>" + str(len(poznati)) + "</b> · ❌ сгрешихме <b>"
                   + str(len(zagubeni)) + "</b>")
    else:
        out.append("Нито един още не е отсъден — мачовете им още не са дошли.")
    if chakat:
        out.append("⏳ " + str(len(chakat))
                   + (" чака мача си" if len(chakat) == 1 else " чакат мачовете си"))
    out += bez_red(len(bez))
    ...
    if _rano:
        out.append("\U0001f5c2 плюс <b>" + str(sum(1 for r in _rano if r.get("hit")))
                   + " от " + str(len(_rano))
                   + "</b> по-раншни карти, отсъдени днес")
    ...
    if fishove:
        ...
        for n in sorted(fishove):
            ...
            red = ("\U0001f3ab Фиш " + str(n) + ": <b>" + str(ok) + " от "
                   + str(len(legs)) + "</b> крака")
            if gotovi < len(legs):
                ost = len(legs) - gotovi
                red += " · " + str(ost) + (" крак още чака" if ost == 1
                                           else " крака още чакат")
            elif ok == len(legs):
...
def selftest():
    ...
    # 🔴 ОБЪРНАТИ 25.08.2026: същите числа, друг градеж — знаменателят пръв.
    check("брои ВСИЧКИ пуснати, не само отсъдените",
          "От <b>7</b> пуснати" in _od)
    ...
    check("знаменателят стои ПРЕДИ числителя",
          predi(_od, "имат резултат", "познати"))
    check("брои познатите", "познати <b>4</b>" in _od)
    check("брои загубените", "загубени <b>1</b>" in _od)
    check("процентът е върху отсъдените",
          "<b>5</b> имат резултат" in _od and "<b>80%</b>" in _od)
    check("казва колко чакат", "1 чака мача си" in _od)
    check("казва колко са без резултат",
          "1 мач без резултат" in _od and "не го броим" in _od)
    ...
    check("празен дневник не гърми", "От <b>0</b> пуснати"
    ...
    check("лайфтайм познати и загубени живеят в равносметката",
          "познати <b>6</b>" in obshto_dosega_text(
              _now, [{"scored": True, "hit": True} for _ in range(6)]
              + [{"scored": True, "hit": False} for _ in range(4)])
          and "загубени <b>4</b>" in obshto_dosega_text(
              _now, [{"scored": True, "hit": True} for _ in range(6)]
              + [{"scored": True, "hit": False} for _ in range(4)]))
    ...
    check("волейболът досега е 12 от 13 — в равносметката",
          "12 от 13 отсъдени" in _dos and "12 от 13" not in _t3)
    check("волейболът досега е 92% — в равносметката",
          "92%" in _dos and "92%" not in _t3)
    check("футболът досега е 2 от 11 — в равносметката",
          "2 от 11 отсъдени" in _dos and "2 от 11" not in _t3)
    check("футболът досега е 18% — в равносметката",
          "18%" in _dos and "18%" not in _t3)
    check("малката извадка НЕ получава процент",
          "1 от 1 отсъдени" in _dos and "1 от 1 отсъдени · <b>" not in _dos)
    check("неотсъденото пак се вижда като пуснато, без процент",
          "ТЕНИС НА МАСА</b> · 1 пуснати · още нищо отсъдено" in _dos)
    ...
    check("честният раздел срещу пазара ОЦЕЛЯВА",
          "Когато не сме съгласни с пазара" in _odp)

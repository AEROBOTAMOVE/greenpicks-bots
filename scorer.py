...
def selftest():
    ...
    check("чужда стая се отказва без мрежа", post("26", "тест") is False)
    ...
    check("познатите отиват при статистиката", _ima("RESULTS", "wins_text"))
    check("познатите НЕ отиват при фишовете", not _ima("WINS", "wins_text"))
...
def main():
    ...
    if wins:
        time.sleep(2.0)
        post(RESULTS_THREAD, wins_text(now, wins))
    else:

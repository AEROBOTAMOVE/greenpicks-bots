...
def bez_red(n, ime=""):
    ...
    # 🔴 ЗА КЛИЕНТА (15.09.2026). Вторият ред казваше «източникът не ги
    # дава» — тоест откъде ботът взима резултатите. Остават фактът и
    # последицата, в един ред.
    return ["\U0001f6ab " + str(n) + mn(n, " мач", " мача")
            + ((" на " + ime) if ime else "") + " без резултат — не "
            + mn(n, "го", "ги") + " броим"]
...
def _t1_ot_puskaneto(posted, t1, minuti=30):
    """True, ако последната точка е от ръна на публикуването (или по-рано).

    И двете са софийско време във вида «ГГГГ-ММ-ДД ЧЧ:ММ». Непознат вид —
    False: съди се както досега.
    """
    from datetime import datetime as _dt, timedelta as _td
    try:
        p = _dt.strptime(str(posted)[:16], "%Y-%m-%d %H:%M")
        z = _dt.strptime(str(t1)[:16], "%Y-%m-%d %H:%M")
    except (TypeError, ValueError):
        return False
    return z <= p + _td(minutes=minuti)
...
def zatvaryashta_ot_redica(r, ceni=None):
    ...
    # 🔴 СМЕНЕН ПАЗАР (15.09.2026). Хокей: първата точка двуизходна (с
    # продълженията), последната — тройка с Х. «1.77 → 2.23» даваше CLV
    # −11.65 т. за нищо. Различен набор изходи = не се мери.
    _c0, _c1 = z.get("c0") or {}, z.get("c1") or {}
    if _c0 and _c1 and sorted(_c0) != sorted(_c1):
        return None
    # 🔴 ТОЧКА ОТ САМОТО ПУСКАНЕ НЕ Е ЗАТВАРЯНЕ (15.09.2026). Редицата може да
    # е почнала в по-ранен рън — тогава последната точка е цената ПРИ
    # пускането и CLV е нула по конструкция (19 от 19 такива в историята).
    if _t1_ot_puskaneto(r.get("posted"), z.get("t1")):
        return None
...
def obshto_dosega_text(now, rows):
    ...
    # 🔴 СЧУПЕНАТА ГРУПА ВЛИЗА В РАВНОСМЕТКАТА (05.09.2026).
    #
    # Пазачът намира групи, които обявяват повече от сбъднатото — измерено:
    # Czech Liga Pro 62.0% срещу 43.3%, тоталите 58.2% срещу 46.4%. Дотук
    # ги печаташе САМО в дневника на Actions, който е нечетим отвън; този
    # проект вече е плащал за такова мълчание («черната кутия», 02.09).
    #
    # Пишат се само ДОКАЗАНИТЕ (обявено ИЗВЪН интервала) и най-много три —
    # равносметката е кратка по избор, а десет реда биха я удавили.
    _lo = []
    try:
        _lo = list(ligi_nechestni(rows)) + [
            (v + " (вид залог)", b, n, ob, sb, d, g)
            for v, b, n, ob, sb, d, g in vidove_nechestni(rows)]
        _lo.sort(key=lambda x: -(x[3] - x[4]))
    except Exception:                                        # noqa: BLE001
        _lo = []
    _redove = []
    if _lo:
        _redove = ["", "\u2696\ufe0f <b>НАДЦЕНЯВАМЕ ТУК</b>"]
        for _lg, _b, _n, _ob, _sb, _d, _g in _lo[:3]:
            _redove.append("· " + str(_lg)[:32] + ": казваме <b>"
                           + ("%.0f" % (100 * _ob)) + "%</b>, сбъдва се <b>"
...
def selftest():
    ...
    # 🔴 15.09.2026: сменен пазар и точка от самото пускане.
    check("смяна двойка → тройка НЕ дава затваряща",
          zatvaryashta_ot_redica(
              {"key": "sm", "pick": "1 · Х"},
              {"sm": {"t0": "2026-09-04 10:00", "t1": "2026-09-04 18:00", "n": 2,
                      "c0": {"1": 1.77, "2": 2.10},
                      "c1": {"1": 2.23, "2": 3.10, "Х": 4.00}}}) is None)
    check("последна точка от рънa на пускането НЕ е затваряне",
          zatvaryashta_ot_redica({"key": "k2", "pick": "1 · Х",
                                  "posted": "2026-09-04 17:50"}, _cen) is None)
    check("точка час след пускането Е затваряне",
          zatvaryashta_ot_redica({"key": "k2", "pick": "1 · Х",
                                  "posted": "2026-09-04 16:00"}, _cen) == 1.70)
    check("без час на пускане се съди както преди",
          zatvaryashta_ot_redica({"key": "k2", "pick": "1 · Х"}, _cen) == 1.70)
    check("непознат вид час не спира мерилото",
          _t1_ot_puskaneto("вчера", "2026-09-04 18:00") is False)
    ...
    # ═══ СЧУПЕНАТА ГРУПА ВЛИЗА В РАВНОСМЕТКАТА (05.09.2026) ══════
    #
    # 🔴 Пазачът печаташе САМО в дневника на Actions, който е нечетим
    # отвън — този проект вече е плащал за такова мълчание («черната
    # кутия», 02.09). Сега влиза в съобщението, което човек чете.
    _o_lazh = [{"scored": True, "hit": (i % 5 == 0), "p": 0.80,
                "league": "Лъжлива", "bucket": "x", "pick": "1 · А",
                "posted": "2026-09-01 10:00"} for i in range(30)]
    _o_chest = [{"scored": True, "hit": (i % 3 != 0), "p": 0.66,
                 "league": "Честна", "bucket": "x", "pick": "1 · А",
                 "posted": "2026-09-01 10:00"} for i in range(30)]
    _o_now = datetime.now(SOFIA)
    _txt_l = obshto_dosega_text(_o_now, _o_lazh + _o_chest)
    _txt_c = obshto_dosega_text(_o_now, _o_chest)
    check("🔴 счупената група влиза в РАВНОСМЕТКАТА",
          "НАДЦЕНЯВАМЕ ТУК" in _txt_l and "Лъжлива" in _txt_l)
    check("и се вижда КОЛКО казваме срещу КОЛКО се сбъдва",
          "казваме" in _txt_l and "сбъдва се" in _txt_l)
    check("честната група НЕ се назовава", "Честна" not in _txt_l)
    # 🔴 ТИШИНАТА ЗНАЧИ «ВСИЧКО Е ЧЕСТНО» и това е информация.
    check("при чисти данни разделът го НЯМА",
          "НАДЦЕНЯВАМЕ ТУК" not in _txt_c)
    check("равносметката пак носи основните си числа",
          "ДОСЕГА ОБЩО" in _txt_c and "с резултат" in _txt_c)
    # 🔴 НАЙ-МНОГО ТРИ РЕДА: десет биха удавили съобщението и
    # пазачът щеше да стане украса.
    _o_mn = []
    for _i in range(6):
        _o_mn += [{"scored": True, "hit": (k % 5 == 0), "p": 0.80 + _i * 0.01,
                   "league": "Л%d" % _i, "bucket": "x", "pick": "1 · А",
                   "posted": "2026-09-01 10:00"} for k in range(30)]
    _txt_m = obshto_dosega_text(_o_now, _o_mn)
    check("не повече от три реда в раздела",
          sum(1 for _r in _txt_m.split(NL) if _r.startswith("· ")) <= 3)
    check("празен дневник не гърми",
          "ДОСЕГА ОБЩО" in obshto_dosega_text(_o_now, []))

    # ═══ ВТОРИЯТ РАЗРЕЗ: ПО ВИД ЗАЛОГ (05.09.2026) ═══════════
    #
    # 🔴 Измерено: тоталите обявяват 58.2% и сбъдват 46.4%
    # [36.2 … 57.0] на 84 карти — надценяване от дванайсет пункта,
    # ДОКАЗАНО. А пазачът по ЛИГА не ги вижда: 39 футболни, 30 бейзболни и
    # 15 баскетболни тотала се удавят всеки в своята лига.
    #
    # Една група може да лъже системно и пак да е невидима, ако разрезът е
    # по друг признак. ЕДИН РАЗРЕЗ НЕ СТИГА.
    check("тоталът се познава по избора",
          vid_zalog({"pick": "Над 2.5 гола"}) == "тотал"
          and vid_zalog({"pick": "Под 8.5 рънове"}) == "тотал")
    check("и по опашката в ключа",
          vid_zalog({"key": "2026-09-05|football|а|б|total"}) == "тотал")
    check("победителят не е тотал",
          vid_zalog({"pick": "1 · Левски"}) == "победител")
    check("празното не гърми", vid_zalog({}) == "победител"
          and vid_zalog(None) == "победител")
    # —— и самият разрез: тотали, пръснати в ТРИ спорта, всеки под прага
    _v_tot = []
    for _b, _n in (("football", 12), ("baseball", 10), ("basketball", 8)):
        _v_tot += [{"scored": True, "hit": (i % 5 == 0), "p": 0.80,
                    "pick": "Над 2.5 гола", "bucket": _b,
                    "league": "Л" + str(i)} for i in range(_n)]
    _v_pob = [{"scored": True, "hit": (i % 3 != 0), "p": 0.66,
               "pick": "1 · А", "bucket": "football",
               "league": "Л" + str(i)} for i in range(30)]
    _naid = {x[0] for x in vidove_nechestni(_v_tot + _v_pob)}
    check("🔴 тоталите се хващат, макар всеки спорт да е под прага",
          "тотал" in _naid)
    check("победителят не се обявява за счупен", "победител" not in _naid)
    # —— и че ЛИГОВИЯТ разрез НАИСТИНА би ги пропуснал (иначе вторият
    # разрез е излишен и проверката трябва да го признае)
    check("лиговият разрез ГИ ПРОПУСКА (затова има втори)",
          not ligi_nechestni(_v_tot + _v_pob))
    check("боклук не гърми",
          vidove_nechestni(None) == [] and vidove_nechestni([1, "х"]) == [])
    # 🔴 И ЧЕ ПЕЧАТЪТ ГИ ПОКАЗВА. Проверките горе викат разреза
    # ПРЯКО; ако `kazhi_nechestnite` не го вика, те пак минават и дефектът
    # оцелява — точно шаблонът «сметката работи, пътят не», който днес ме
    # хвана пет пъти. Тук се брои изходът на самия печат.
    check("🔴 печатът показва и разреза по вид залог",
          kazhi_nechestnite(_v_tot + _v_pob) >= 1)
    check("а при чисти данни мълчи",
          kazhi_nechestnite(_v_pob) == 0)
    ...
    # 🔴 ОБЪРНАТА (15.09.2026): откъде идват резултатите не е за клиента.
    check("редът НЕ казва откъде идват резултатите",
          "източник" not in NL.join(_b))
    check("но казва последицата", "не ги броим" in NL.join(_b))
    ...
    # 🔴 ОБЪРНАТИ (15.09.2026): последицата остава и в двете съобщения,
    # откъде идват резултатите — не (то не е за клиента).
    check("обзорът казва последицата за неотсъдимите",
          "не ги броим" in _tmn and "източник" not in _tmn)
    check("финишът я казва по СЪЩИЯ начин",
          "не го броим" in _fin and "източник" not in _fin)
...
def vid_zalog(rec):
    """«тотал» или «победител» — грубо, но достатъчно за разрез.

    Тоталът се познава по НАЧАЛОТО на избора («Над 2.5 гола»), а не по
    ключа: ключът носи опашка «|total» само за втората карта от същия мач,
    а тоталите идват и по друг път.
    """
    p = str((rec or {}).get("pick") or "").strip()
    if p.startswith(("Над ", "Под ")):
        return "тотал"
    if "|total" in str((rec or {}).get("key") or ""):
        return "тотал"
    return "победител"


def vidove_nechestni(rows, min_n=None):
    """Същото като `ligi_nechestni`, но разрязано по ВИД ЗАЛОГ.

    🔴 ЗАЩО ВТОРИ РАЗРЕЗ. Измерено: тоталите обявяват 58.2% и сбъдват
    46.4% [36.1 … 57.0] на 84 карти — надценяване от дванайсет пункта,
    доказано. Пазачът по ЛИГА не ги вижда, защото 39 футболни, 30
    бейзболни и 15 баскетболни тотала се удавят всеки в своята лига.

    Една група може да лъже системно и пак да е невидима, ако разрезът е по
    друг признак. Един разрез не стига.
    """
    prag = LIGA_MIN if min_n is None else int(min_n)
    po = {}
    for r in (rows or []):
        ...
        if not (0.0 < p < 1.0):
            continue
        # 🔴 ДВА КЛЮЧА, НЕ ЕДИН. Тоталите са ОБЩ модел за всички
        # спортове, а разрязани по спорт се разпадат на 39 футболни, 30
        # бейзболни и 15 баскетболни — всяка под прага, тоест невидими.
        # Затова се брои и видът САМ, и видът в рамките на спорта: първото
        # хваща общата повреда, второто — спорта, в който е най-зле.
        _v = vid_zalog(r)
        po.setdefault((_v, "всички"), []).append((p, bool(r.get("hit"))))
        po.setdefault((_v, str(r.get("bucket") or "?")),
                      []).append((p, bool(r.get("hit"))))
    out = []
    for (vid, b), v in po.items():
        n = len(v)
        if n < prag:
            continue
        k = sum(1 for _p, h in v if h)
        ...
        lo, hi = _wilson(k, n)
        if ob > hi:
            out.append((vid, b, n, ob, float(k) / n, lo, hi))
    out.sort(key=lambda x: -(x[3] - x[4]))
    return out
...
def kazhi_nechestnite(rows):
    ...
    if not LIGA_VKL:
        return 0
    lo = list(ligi_nechestni(rows))
    # 🔴 ВТОРИЯТ РАЗРЕЗ. Тоталите са пръснати във всяка лига и по лига са
    # невидими: измерено, обявяват 58.2% и сбъдват 46.4% на 84 карти.
    lo += [(v + " (вид залог)", b, n, ob, sb, d, g)
           for v, b, n, ob, sb, d, g in vidove_nechestni(rows)]
    lo.sort(key=lambda x: -(x[3] - x[4]))

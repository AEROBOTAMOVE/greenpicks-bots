...
NOVI = {
    ...
    # 🎮 ЕЛЕКТРОННИТЕ СПОРТОВЕ (26.08.2026). Модулът esport.py е готов —
    # 109 проверки, 31 живи мача, три отделни игри със СВОЯ статистика.
    # Стаята е последното, което му липсва: predictor.py нарочно няма
    # закован номер за него, за да се влее сам оттук през rooms_state.json.
    #
    # Стои СЪС затворено обяснение, защото спортът пуска карти чак когато
    # се намери източник за резултати (PREDICT_ESPORT_MAX=0 днес). Празна
    # стая без обяснение е по-лоша от липсваща — читателят я отваря и не
    # разбира защо мълчи.
    "esports": ("🎮 Електронни спортове", 13338331, [
        "🎮 <b>ЕЛЕКТРОННИ СПОРТОВЕ — стаята е готова, картите чакат</b>",
        "Тук ще влизат прогнозите за <b>CS2, League of Legends и Valorant</b> —"
        " всяка игра със своя сметка, защото три различни игри в едно число"
        " не значат нищо.",
        "Още не пускаме карти по една причина: намерихме цените, но не и"
        " <b>резултатите</b>. Карта, която никой не може да отсъди, виси тук"
        " завинаги — това е по-лошо от липсваща карта. Намери ли се източник,"
        " стаята тръгва сама.",
    ]),
...
def selftest():
    ...
    # 🔴 ПРОВЕРКАТА СЕ ОБРЪЩА ЗАЕДНО С ПРАВИЛОТО, НЕ СЕ ТРИЕ (26.08.2026).
    # Беше „четири нови спорта". Добавен е ПЕТИЯТ — електронните спортове.
    # Боксът НЕ влиза тук: той дели стаята на ММА (328), както е записано в
    # predictor.py, и нова стая не му трябва.
    check("пет нови спорта", len(NOVI) == 5)
    check("боксът НЕ иска своя стая — дели тази на ММА", "boxing" not in NOVI)
    for s in ("hockey", "tennis", "baseball", "amfootball", "esports"):
    ...
    check("вече създадената стая не се брои за липсваща",
          "hockey" not in _lipsvat and len(_lipsvat) == len(NOVI) - 1)

...
ЖЕЛЕЗНИ ПРАВИЛА (не се изключват с env)
  ...
  - Контейнерите на темите {1,3,4,5,6,7,8,9,10,11,26,27,328} не се трият никога.
...
# 🔴 ВИКА СЕ ТУК, НА НИВО МОДУЛ — 11.08.2026.
#
# Функцията горе беше НАПИСАНА, но НИКОГА НЕ СЕ ВИКАШЕ (grep: нула повиквания).
# Тоест защитата съществуваше само на хартия: KEEP_IDS оставаше закованият
# списък от 13 номера, а стаите 1961, 1963, 1965 и 1967 бяха извън него.
# Този файл ТРИЕ съобщения по номер. Изтрита форум-тема НЕ се връща.
#
# Собствената самопроверка на файла го хващаше и крещеше пет червени реда —
# просто никой не я беше пуснал. Затова вече се вика при внасяне на модула,
# преди която и да е функция да е видяла KEEP_IDS.
_ZASHTITENI = _vlei_stai_ot_fayla()
if _ZASHTITENI:
    print("Защитени допълнителни стаи от rooms_state.json: "
          + ", ".join(s + " (" + str(n) + ")" for s, n in sorted(_ZASHTITENI,
                                                                key=lambda x: x[1])))
...
ROOM_NAME = {
    ...
    26: "Новини", 27: "БОТА ПРЕДРИЧА", 328: "Бойни спортове",
}

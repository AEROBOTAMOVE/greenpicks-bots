...
ROOM_PINS = {
    ...
    328: block(
        "🥊 <b>БОЙНИ СПОРТОВЕ</b>",
        "Тук влизат <b>прогнозите за бойни спортове</b>: UFC и PFL — бойци, избор, числата. Бокс няма: за него не намерихме отворен източник и не измисляме.",
        "Новините са в стая 📰 Новини · всички спортове заедно — в 🤖 БОТА ПРЕДРИЧА."),
}
...
NOVI_STAI_TEKST = {
    # 11.08.2026 — ЗАТВОРЕНИ ДО СТАРТА НА СЕЗОНА.
    # Стените обещаваха прогнози, а дневникът показа 0 карти от 261. Пинът
    # вече казва истината, за да не се пита никой какво става.
    "hockey": block(
...
def selftest():
    ...
    for _s in (_zatv or ()):
        ...
        for _n, _txt in ROOM_PINS.items():
            check("пинът на стая " + str(_n) + " не обещава " + _s,
                  _e not in _txt or _txt == NOVI_STAI_TEKST.get(_s))
    ...
    # Пазач срещу връщането на обещанието. Двата затворени спорта НЕ бива да
    # казват „тук влизат прогнозите" — точно това караше собственика да пита
    # защо стаята е празна. Тестван и в обратната посока: махнеш ли думата
    # „затворено", проверката пада.
    for s in ("hockey", "amfootball"):
        _txt = NOVI_STAI_TEKST[s]
        check(s + ": стената казва, че е затворено", "затворено" in _txt)
        check(s + ": стената НЕ обещава прогнози",
              "Тук влизат <b>прогнозите" not in _txt)
        check(s + ": стената казва накъде да гледа човекът",
              "БОТА ПРЕДРИЧА" in _txt)
    for s in ("tennis", "baseball"):
        check(s + ": стената ОБЕЩАВА прогнози (спортът играе)",
              "Тук влизат <b>прогнозите" in NOVI_STAI_TEKST[s])

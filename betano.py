...
TABLICA = {
    ...
    # 🔴 «ц» -> «z», НЕ «c» (поправено 08.09.2026). През «c» то попадаше в
    # свиването c -> k и «Грац» ставаше «grak», «Цюрих» — «kiurih». Но «ц» е
    # звукът «ts»/«z» и никога «k»: Graz се чете «Гратс», Zurich — «Цюрих».
    "х": "h", "ц": "z", "ч": "ch", "ш": "sh", "щ": "sht", "ъ": "a",
...
# 🔴 «sk» ПРЕД ПРЕДНА ГЛАСНА ЗВУЧИ «sh». Шведското «Skellefteå» е
# «Шелефтео» и Betano го пише фонетично. Измерено: +2 верни съвпадения,
# нула нови лъжи, нула адверсарна цена.
_SK_PRED_E = re.compile(r"sk(?=[eiy])")
...
def selftest():
    ...
    # 🔴 «ц» Е «z», НЕ «k». През «c» то попадаше в свиването c -> k и «Грац»
    # ставаше «grak». Намерено от жива карта: «Айзбарен Берлин — Грац».
    check("кирилското «ц» става «z»", latinica("Цюрих").startswith("z"))
    check("и НЕ става «k»", "k" not in latinica("Цюрих"))
    check("Zurich среща Цюрих", sreshta("Zurich", "Цюрих"))
    check("но «ц» не слива всичко",
          not sreshta("Цюрих", "Мюнхен"))
    # 🔴 «sk» ПРЕД ПРЕДНА ГЛАСНА = «sh». Намерено от жива карта:
    # «Шелефтео Аик — Женева Сервет» срещу «Skellefteå AIK».
    check("«sk» пред «e» става «sh»",
    ...
    check("и не слива различни", not sreshta("Skellefteå AIK", "Шелби Юнайтед"))

...
#   срещу която стои.
# Вдигана заедно с всяко истинско разширение, за да не може броят тихо да
# падне: 4500 → 4870 (01.09) → 5600 (02.09, след еврохокея и червения
# списък). Числото е ПОД текущото, не над него — прагът е под, не цел.
DOLNA_GRANITSA = int(os.environ.get("TESTOVE_DOLNA_GRANICA") or "5800")
...
ZADALZHITELNI = (
    ...
    "reset",
    # 🏒 Нов спорт, нов задължителен модул (02.09.2026). Ако утре
    # `evrohokey.py` спре да се внася или самопроверката му падне, това се
    # чува ТУК, а не когато хокейните карти тихо станат нула.
    "evrohokey",
    # 🎯 Нов източник на коефициенти (08.09.2026). Падне ли самопроверката
    # му, това се чува ТУК, а не когато коефициентите тихо станат нула.
    "kambi_ceni",
    # 🇧🇬 Българската книга (08.09.2026).
    "betano",
    # 🏉 Нов спорт, нов задължителен модул (02.09.2026). Ако утре
    # `ragbi.py` спре да се внася, това се чува ТУК, а не когато
    # ръгби картите тихо станат нула.
    "ragbi",
)
...
# 🔴 БАВНИТЕ ПО ИМЕ (08.09.2026). cyalost сверява всеки от 83-те белега с
# живия код на всички модули — работата му расте заедно с файловете. Измерено
# в тази сесия: 212 сек при таван 180, тоест пакетът го обявяваше за «заби» и
# го слагаше в «ЗА ОПРАВЯНЕ», докато той минава чист. Пазач, който вика за
# здрав модул, учи човека да подминава и истинските тревоги.
#
# Таванът НЕ се маха — без него един наистина забил модул би заковал пакета.
BAVNI = {"cyalost": 600}


def pusni(ime, papka=None, timeout=None):
    if timeout is None:
        timeout = BAVNI.get(ime, 180)

...
VAZHNI = (
    ...
    ("news.yml", "новините"),
    # 🔴 matches.yml МАХНАТ от надзора (15.09.2026): собственикът сви групата
    # до три теми (Бота предрича, Фишове на деня, Резултати) и изтри стаите по
    # спорт, които matches_bot пълнеше. Кронът му е спрян нарочно; здравният
    # преглед не бива да го чака (иначе крещи за workflow, който няма да върви).
    # ПЪТ НАЗАД: върни реда тук И крона в matches.yml.
    ("support.yml", "съпортът и будилникът"),
    ("pazach.yml", "самият пазач"),
    # 🔴 ДОБАВЕН 02.09.2026 — беше ЕДИНСТВЕНИЯТ workflow с крон, който няма
    # НИТО портиер, НИТО надзор. Одит на всичките 16: всеки друг с крон има и
    # двете. Хъбът опреснява стените в стаите; замълчи ли, те застояват тихо.
    # Защо чак сега: при стария плосък гратис (период + 100 мин) допускът му
    # беше 25.7 ч, а последните 10 паузи стигат 34.3 ч — тоест щеше да лъже.
    # С гратиса по будителя допускът е 36 ч и последните 10 паузи минават
    # (23.2 22.8 24.6 23.1 18.7 25.1 34.3 24.0 23.9 24.1).
    # Трите му истински мълчания (204.5 ч, 155.6 ч, 150.1 ч) са от юли и
    # началото на август — точно тях искаме да хваща.
    ("hub.yml", "хъбът опреснява стените"),
)
...
# 🔴 ТРИ СПОРТА ЛИПСВАХА (добавени 02.09.2026). Сверено срещу
# `predictor.SPORT_ORDER`: прегледът знаеше девет, предсказателят има
# дванайсет. Еспортът не е дребен пропуск — живият дневник има 64 негови
# карти, тоест спорт с шейсет и четири карти можеше да занемее и прегледът
# нямаше да го спомене. Емоджитата са СЪЩИТЕ като при предсказателя, за да
# чете човек едно и също име в двете съобщения.
IME = {
...
# 🔴 ХОКЕЯТ ИЗЛЕЗЕ ОТТУК (02.09.2026), и то по ДВЕ причини:
#   1. редът твърдеше „НХЛ отваря около 15.09" — НЕВЯРНО. Снето живо от
#      api-web.nhle.com: 29.09.
#   2. по-важното: хокеят ВЕЧЕ НЕ Е извън сезон. `evrohokey.py` носи СМ Лига
#      и Шампионската хокейна лига — 36 насрочени срещи, измерено, и живият
#      рън в 09:23 показа 36 сурови / 24 годни.
# Спорт, който произвежда карти, няма работа в списъка „празно е нормално":
# точно този списък би заглушил истинската тревога, ако утре занемее.
#
# Амер. футболът също е отворен от 01.09, но редът му ОСТАВА: НФЛ наистина
# дава предсезонни до 10.09 и гейтът наистина ги реже — тоест мълчание в
# този прозорец Е нормално и има измерена причина.
IZVAN_SEZONA = {
...
def sadi_workflow(ime, opis, rr, dopusk, sega):
    ...
    ot = d["ot_uspeh_ch"]
    sufiks = ("" if ot is None else " · последен успех преди %.1f ч" % ot)
    if d["poredni"]:
    ...
    if ot is not None and ot * 60.0 > dopusk:
        # 01.09.2026. Тук пишеше «дължи на всеки %.0f ч» и подаваше ДОПУСКА.
        # Измерено живо: pazach.yml има крон на всеки 60 мин, а редът обявяваше
        # «на всеки 3 ч» — това е ДОПУСКЪТ (60 + 100 мин гратис), закръглен.
        # Числото и думата до него си противоречаха; заради това 12 поредни
        # ЗЕЛЕНИ рънa на пазача се четяха като паднал workflow.
        # Сега трите числа са РАЗДЕЛЕНИ и назовани: мълчание, крон, допуск.
        # ВНИМАНИЕ: началото «не е успявал от» е ИГЛА в daily.yml (TEZHKI).
        # Смени ли се то, редът пада в клона «непознат» и пак се брои за тежък,
        # но иглата изгнива МЪЛЧАЛИВО. Затова началото НЕ се пипа.
        _kron_ch = max(0.0, (dopusk - GRATIS_MIN) / 60.0)
        return _kaz("mulchi", "не е успявал от %.1f ч · кронът му е на всеки"
                              " %.1f ч, а допускът е %.1f ч (крон + %d мин"
                              " гратис)" % (ot, _kron_ch, dopusk / 60.0,
                                            GRATIS_MIN))
...
def selftest():
    ...
    # ── 3б. ТЕКСТЪТ НА МЪЛЧАНИЕТО не бива да лъже (01.09.2026) ───────────
    # Живо: pazach.yml е с крон на всеки 60 мин, а редът обявяваше «на всеки
    # 3 ч». Трите числа трябва да се четат поотделно, и при гратис над нулата
    # кронът НЕ МОЖЕ да е равен на допуска.
    d = sadi_workflow("pazach.yml", "пазачът", [_run("success", 40, sega)],
                      60 + GRATIS_MIN, sega)
    _kk = d["kak"]
    check("мълчанието пази иглата на daily.yml", "не е успявал от" in _kk)
    check("мълчанието назовава крона отделно", "кронът му е на всеки" in _kk)
    check("мълчанието назовава допуска отделно", "допускът е" in _kk)
    check("мълчанието казва и гратиса",
          ("%d мин гратис" % GRATIS_MIN) in _kk)
    check("кронът е ИСТИНСКИЯТ, не допускът",
          "на всеки 1.0 ч" in _kk
          and ("допускът е %.1f ч" % ((60.0 + GRATIS_MIN) / 60.0)) in _kk)
    check("двете числа се РАЗЛИЧАВАТ, щом има гратис",
          GRATIS_MIN <= 0 or "допускът е 1.0 ч" not in _kk)
    d = sadi_workflow("s.yml", "оценителят",
                      [_run("failure", 1, sega), _run("success", 43, sega)],
                      900 + GRATIS_MIN, sega)
    check("часът на последния успех е с десетична", "преди 43." in d["kak"])

    # ВРЪЗКА С daily.yml: неговата стъпка «Тежко ли е намереното» вече НЕ се
    # самоосъжда, и я разпознава по образеца `^   • daily.yml`. Промени ли се
    # тук първата дума на присъдата, връзката се къса МЪЛЧАЛИВО и ключалката
    # се връща. Затова тя се пази ТУК, а не се открива червена след ден.
    d = sadi_workflow("daily.yml", "дневният бот", [_run("failure", 2, sega)],
                      24 * 60 + GRATIS_MIN, sega)
    check("присъдата за daily.yml започва с името на файла",
          d["tekst"].startswith("daily.yml"))
    ...
    # 🔴 ХЪБЪТ (02.09.2026). Одитът на 16-те workflow-а показа, че той беше
    # единственият с крон и БЕЗ надзор. Проверката пази да не изпадне пак.
    check("хъбът е под надзор", "hub.yml" in [i for i, _ in VAZHNI])
    # И обратната посока: всеки следен ТРЯБВА да има разбираем крон, инак
    # прагът му е None и «не знам» тихо минава за «наред».
    check("всеки следен има разбираем крон",
          not [i for i, _ in VAZHNI if dopusk_min(i) is None])
    # Тънкият запас е ИЗМЕРЕН, не преценен: последните 10 паузи на хъба
    # стигат 34.3 ч, а допускът му е 36 ч. Падне ли под 35, той пак ще лъже.
    check("прозорецът на хъба покрива измерените 34.3 ч",
          (dopusk_min("hub.yml") or 0) > 34.5 * 60)
    ...
    # ═══════ СПОРТОВЕТЕ НА ДВАТА ФАЙЛА СЪВПАДАТ (02.09.2026)
    #
    # 🔴 НАМЕРЕНО ЖИВО: `IME` знаеше ДЕВЕТ спорта, а `predictor.SPORT_ORDER`
    # има ДВАНАЙСЕТ. Липсваха boxing, esports и rugby — а еспортът има 64
    # карти в живия дневник. Тоест спорт с шейсет и четири карти можеше да
    # занемее и прегледът просто нямаше да го спомене.
    #
    # `predictor.py` се чете като ТЕКСТ, не се внася: той е 1 МБ и дърпа
    # десет модула, а този преглед трябва да тръгва И когато предсказателят
    # е счупен — точно тогава е най-нужен.
    _sv_p = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         "predictor.py")
    try:
        with open(_sv_p, encoding="utf-8-sig") as _fh:
            _sv_t = _fh.read()
    except (OSError, UnicodeDecodeError):
        _sv_t = ""
    check("predictor.py се прочете за сверка", bool(_sv_t))
    if _sv_t:
        _i = _sv_t.find("SPORT_ORDER = [")
        check("SPORT_ORDER се намери в predictor.py", _i >= 0)
        if _i >= 0:
            _blok = _sv_t[_i:_sv_t.find("]", _i) + 1]
            _negovi = set()
            for _p in _blok.split('"'):
                _p = _p.strip()
                if _p and _p.isalpha() and _p.islower() and len(_p) > 2:
                    _negovi.add(_p)
            check("извадиха се поне десет спорта", len(_negovi) >= 10)
            _lipsvat = sorted(_negovi - set(IME))
            check("всеки спорт на предсказателя има име тук (%s)"
                  % (", ".join(_lipsvat) or "-"), not _lipsvat)
            # И обратната посока: име тук за спорт, който предсказателят не
            # знае, значи прегледът чака нещо, което никога няма да дойде.
            _izlishni = sorted(set(IME) - _negovi)
            check("няма име за спорт, който предсказателят не знае (%s)"
                  % (", ".join(_izlishni) or "-"), not _izlishni)
    # 🔴 И ХОКЕЯТ НЕ Е ВЕЧЕ „ИЗВЪН СЕЗОНА". `evrohokey.py` носи СМ Лига и

...
def room_card(bucket, rows, now):
    ...
    # 🔴 ЗА КЛИЕНТА (15.09.2026): опашката обясняваше подредбата на стаите и
    # бота. Остава само подписът.
    _ = only
    tail = "🟢 THE GREEN ROOM"
...
def predict_card(picks, now, news_titles):
    ...
    for i, fx in enumerate(picks):
        ...
        # 🔴 ЗА КЛИЕНТА (15.09.2026). Оттук идваше ВТОРИ процент за същия мач
        # («около 58% срещу 42%») и «База: само 2 мача». Процентът е на
        # картата в същата стая; тук остава само фактът за сезона.
        parts += [r for r in numbers_say(fx["home"], fx["away"], st)
                  if r.startswith("📊")]
        parts.append("")

    # 🔴 ЗА КЛИЕНТА (15.09.2026): упътванията «Как се чете това» и «Малка
    # извадка = предпазливо число» паднаха заедно с втория процент.

...
def numbers_say(home, away, st):
    ...
    # Без никакви данни: домакинско предимство, малък и честен наклон.
    if n < 1:
        return [f"🎲 Числата: лек превес за <b>{esc(home)}</b> — около 54% срещу 46%.",
                "   База: домакинско предимство (за тази среща нямаме изиграни мачове)."]
...
def selftest():
    ...
    check("празна извадка ВСЕ ПАК дава прогноза", "превес" in ns_small[0])
    check("празната извадка е призната на глас",
          "нямаме изиграни мачове" in " ".join(ns_small))
    check("празната извадка не обещава много", "54%" in ns_small[0])

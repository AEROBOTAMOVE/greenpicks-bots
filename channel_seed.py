...
WELCOME = block(
    ...
    # 🔴 ЗА КЛИЕНТА (15.09.2026): без «смятат от статистика», без извадката
    # и «6 и 5 мача», без «тънки данни», без остарелите часове 08:00–23:00.
    "Прогнози за мачовете от деня: изборът, процентът и цената на мача. "
    "Нищо не се трие — сгрешеното стои до познатото.",
    "",
    "В канала: фишовете на човека зад тях и резултатите на деня. Всичко "
    "останало е в групата — прогнозите по стая за всеки спорт, новините и "
    "пълната статистика. 👇 Влизането е от бутона.",
    "",
...
def selftest():
    ...
    for it in POSTS:
        ...
        # 🔴 ОБЪРНАТА (15.09.2026): вътрешността НЕ се връща в поста.
        for _dum in ("извадк", "поасон", "модел", "стъпва", "08:00", "23:00",
                     "тънки данни"):
            if _dum in low:
                problems.append(name + ": вътрешност към клиента — „" + _dum + "“")
        for tag in ("b", "i"):

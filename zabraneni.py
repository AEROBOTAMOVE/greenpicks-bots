...
# 🔴 «КОЕФ» ИЗЛЕЗЕ ОТ ЗАБРАНЕНИТЕ (02.09.2026, по изрично нареждане).
#
# Собственикът, дословно: «тука цени няма, има КОЕФИЦИЕНТИ» и «да го пише
# заедно с прогнозата». Измерено същия ден: числото Е на 48 от 48 карти,
# просто беше описано («толкова пъти се връща вложеното») вместо назовано,
# защото тази дума не минаваше през пазача.
#
# ИМЕНАТА НА БУКМЕЙКЪРИТЕ ОСТАВАТ ЗАБРАНЕНИ — всичките 53 в IMENA. Тях
# никой не е искал и същинският риск е там. «залагай», «заложи», «18+»,
# «букмейкър», «odds», «финансов съвет», «гарантирана печалба», «сигурен
# залог» също остават. Пада само името на едно число, което вече се публикува.
#
# ПЪТ НАЗАД: GREENPICKS_ZABRANI_KOEF=1 връща думата в списъка.
ZABRANI_KOEF = (os.environ.get("GREENPICKS_ZABRANI_KOEF") or "0").strip() in (
    "1", "true", "yes", "да")

DUMI = ([] if not ZABRANI_KOEF else ["коеф"]) + [
    ...
    "залагай", "заложи", "18+", "букмейкър", "odds",
...
def selftest():
    ...
    # ══════════════════ ДУМАТА «КОЕФИЦИЕНТ» (02.09.2026)
    #
    # Собственикът я поиска поименно, два пъти. Числото ВЕЧЕ беше на 48 от
    # 48 карти, но се описваше вместо да се назовава, защото «коеф» стоеше
    # в забранените. Проверките долу заковават КАКВО ПАДА И КАКВО ОСТАВА —
    # и двете посоки, защото пазач, изпитан само в едната, е половин пазач.
    check("думата «коефициент» вече минава",
          banned_word("💰 коефициент 1.85") is None)
    check("и в пълния вид на реда",
          banned_word("💰 <b>1.14</b> · коефициент · "
                      "толкова пъти се връща вложеното") is None)
    check("и с главна буква", banned_word("Коефициент: 1.90") is None)

    # 🔴 А ТЕЗИ ТРЯБВА ДА ПАДАТ И СЛЕД ПРОМЯНАТА. Изписани са дословно, а не
    # взети от списъците: инак махането на дума от списък щеше да мине.
    for _losh in ("bet365", "bet365 дава 2.10", "виж го в бет365",
                  "pinnacle", "Пинакъл", "DraftKings", "Уинбет", "Palms Bet",
                  "efbet", "залагай отговорно", "заложи сега", "18+",
                  "букмейкър", "odds 1.90", "финансов съвет",
                  "гарантирана печалба", "сигурен залог"):
        check("пак пада: " + _losh, banned_word(_losh) is not None)

    check("имената на букмейкърите са непокътнати", len(IMENA) >= 50)
    check("«коеф» НЕ е в списъка по подразбиране", "коеф" not in DUMI)
    check("другите хазартни думи ОСТАВАТ",
          {"залагай", "заложи", "18+", "букмейкър", "odds"} <= set(DUMI))
    check("правилото за връщане е ИЗКЛЮЧЕНО по подразбиране",
          ZABRANI_KOEF is False)

    # ── и ПЪТЯТ НАЗАД да работи наистина, не само на думи.
    # 🔴 Кешът `_KESH` се чисти НАРОЧНО: без това списъкът се сменя, а
    # пазачът продължава да съди по стария и проверката минава на празно.
    _st_dumi = list(DUMI)
    try:
        globals()["DUMI"] = ["коеф"] + _st_dumi
        globals()["ZABRANENI"] = IMENA + globals()["DUMI"]
        _KESH.clear()
        check("с върнатата дума пазачът пак я хваща",
              banned_word("коефициент 1.85") is not None)
        check("и картата пак би загубила името",
              banned_word("💰 <b>1.14</b> · коефициент") is not None)
    finally:
        globals()["DUMI"] = _st_dumi
        globals()["ZABRANENI"] = IMENA + _st_dumi
        _KESH.clear()
    check("списъкът е върнат както беше", "коеф" not in DUMI)
    check("и пазачът пак пуска думата",
          banned_word("коефициент 1.85") is None)

...
def main():
    # 🔴 ОБЕЗВРЕДЕН НА 11.08.2026. Този файл праща НОВИНИ В КАНАЛА — точно това,
    # което желязното правило на групата забранява („📌 Новини в канала НЯМА",
    # setup_hub.py). Всички новини живеят в стая 26 и се пускат от news_bot.py.
    #
    # За разлика от news_showcase.py и edu_seed.py, които са надписани като
    # пенсионирани, тук нямаше нито пазач, нито работен файл, нито предупреждение
    # — тоест едно пускане по навик изсипваше дванайсет новини в канала пред
    # всички абонати, без път назад. Заредено оръжие в чекмеджето.
    #
    # Кодът остава непокътнат под този ред: ако някой ден каналът получи своя
    # новинарска секция, махаш стопера и файлът тръгва. Дотогава — мълчи.
    if os.environ.get("SEED_NEWS_I_KNOW_WHAT_I_AM_DOING") != "да":
        print("seed_news е спрян: новини в КАНАЛА нарушават правилото на групата.")
        print("Новините се пускат от news_bot.py и живеят само в стая 26.")
        print("Ако наистина трябва: SEED_NEWS_I_KNOW_WHAT_I_AM_DOING=да")
        return 0

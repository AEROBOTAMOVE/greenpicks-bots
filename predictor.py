...
# Под този праг не твърдим сам победител, а даваме двоен шанс (1Х / Х2).
# 0.50 не е избрано на око: под половината значи, че по-вероятно е НЕ това да
# стане, отколкото това. Такова твърдение не бива да излиза само.
# Пълната сметка защо изобщо съществува двойният шанс стои при pick_1x2.
FOOT_SINGLE_MIN = 0.50
...
def card(an, now):
    ...
    if stars <= 1:
        ...
        # Едната звезда има ДВЕ различни причини (grade, ред 1042): или извадката
        # е под 10 (n_eff), или превесът е слаб. Сега се казва вярната.
        star_line += (" · малка извадка" if float(an.get("n_eff") or 0.0) < 10.0
                      else " · слаб превес")
    lines += ["",
...
def selftest():
    ...
    # --- ДВОЙНИЯТ ШАНС. Причината стои изписана при pick_1x2: единайсет
    # футболни прогнози, две познати, четири загуби от чисти равенства.
    _dc = pick_1x2(0.42, 0.30, 0.28, "Арсенал", "Челси")
    check("под 50% домакинът става 1Х", _dc[0] == "1Х · Арсенал или равен")
    check("1Х събира двете вероятности", abs(_dc[1] - 0.72) < 1e-9)
    check("1Х се мери срещу база 2/3", abs(_dc[2] - 2.0 / 3.0) < 1e-9)
    _dc2 = pick_1x2(0.28, 0.30, 0.42, "Арсенал", "Челси")
    check("под 50% гостът става Х2", _dc2[0] == "Х2 · Челси или равен")
    check("Х2 събира двете вероятности", abs(_dc2[1] - 0.72) < 1e-9)
    check("точно на 50% остава единичен",
          pick_1x2(0.50, 0.30, 0.20, "Арсенал", "Челси")[0] == "1 · победа Арсенал")
    check("двойният шанс не използва израза НЕ ГУБИ",
          "не губи" not in _dc[0] and "не губи" not in _dc2[0])
    ...
    _ft = fallback_analyse({"bucket": "football", "emoji": "⚽", "home": "Първи",
                           ...
                           {}, "няма история")
    check("резервният футбол дава двоен шанс, а не гол победител",
          _ft["pick"].startswith("1Х ·"))
    check("резервният футбол е над 0.5, защото покрива два изхода",
          _ft["p"] > 0.5)
    ...
    check("тенисът на маса е изключен от фишовете",
          "tabletennis" in COMBO_NO_RESULT)
    ...
    _pul = [a for a in _kand
            if float(a.get("p") or 0.0) >= COMBO_MIN_P
            and a.get("bucket") not in COMBO_NO_RESULT]
    check("филтърът маха тениса на маса въпреки най-високото p", len(_pul) == 2)
    ...
    check("малката извадка се назовава малка",
          "малка извадка" in _karta(1, 4.0, "извадка 3+3 мача"))
    check("голямата извадка НЕ се нарича малка",
          "малка извадка" not in _karta(1, 135.0, "извадка 135+136 мача"))
    check("слабият превес се назовава слаб превес",
          "слаб превес" in _karta(1, 135.0, "извадка 135+136 мача"))
    check("картата с три звезди няма нито едното",
          "малка извадка" not in _karta(3, 135.0, "извадка 135+136 мача")
          and "слаб превес" not in _karta(3, 135.0, "извадка 135+136 мача"))
    check("прагът е точно 10 — под него е малка извадка",
          "малка извадка" in _karta(1, 9.9, "извадка 5+5 мача")
          and "слаб превес" in _karta(1, 10.0, "извадка 20+20 мача"))
    ...
    check("гол-гол: ДА при 62%", btts_line(0.62) == "Гол-гол: ДА <b>62%</b>")
    check("гол-гол: НЕ при 42% отгоре", btts_line(0.42) == "Гол-гол: НЕ <b>58%</b>")
    check("гол-гол: между праговете мълчи",
          btts_line(0.50) == "" and btts_line(0.44) == "")
    check("гол-гол: прагът 57% е включително",
          btts_line(0.57) != "" and btts_line(0.43) != "")
    ...
    check("едностранен мач: много голове, но НЕ и от двамата",
          over_under_line(_mk_1s["p_over"]).startswith("Над")
          and btts_line(_mk_1s["p_btts"]).startswith("Гол-гол: НЕ"))

...
def selftest():
    ...
    _v20 = datetime(2026, 8, 5, 20, 5, tzinfo=SOFIA)      # вечерното пускане
    _v10 = datetime(2026, 8, 5, 10, 5, tzinfo=SOFIA)      # сутрешното
    check("след 20:00 следващото пускане е утре в 8",
          next_run(_v20).hour == 8 and next_run(_v20).day == 6)
    check("в 10:05 следващото пускане е в 12:00", next_run(_v10).hour == 12)
    check("часовете на пускане са седем", len(RUN_HOURS) == 7)
    check("първото е 8, последното 20",
          RUN_HOURS[0] == 8 and RUN_HOURS[-1] == 20)

    def _fx(chas, den=5):
        return {"when": datetime(2026, 8, den, chas, 0, tzinfo=SOFIA)}

    check("мач в 02:00 през нощта Е спешен вечерта",
          urgent(_fx(2, 6), _v20) is True)
    check("мач в 23:00 Е спешен вечерта", urgent(_fx(23), _v20) is True)
    check("мач утре в 19:00 НЕ е спешен вечерта",
    ...
    _tri = [mk2("football", 3, 0.9, 23), mk2("tennis", 1, 0.1, 21),
            mk2("volleyball", 2, 0.5, 22)]

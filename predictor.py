...
HORIZONT_PO_SPORT = {
    ...
    # RUGBY: 120 ч, по същата причина като хокея и амер. футбола (08.09.2026).
    #
    # Ръгбито е СЕДМИЧЕН спорт: Топ 14 играе в събота, НПС през уикенда.
    # При общия хоризонт от 30 часа всичките 16 събрани срещи стояха
    # «далече» и в кофата влизаха НУЛА — здравният преглед го вика от дни:
    # «има срещи, няма карти».
    #
    # Измерено сега: най-близката е след 52 ч, най-далечната след 108 ч.
    # Тоест 30 ч не хваща НИТО ЕДНА, а 120 ги хваща всичките.
    "rugby": env_int("PREDICT_HORIZON_RAG", 120, 2, 336),
...
# 💵 РАЗМЕРЪТ НА ЗАЛОГА (05.09.2026) — виж дългото обяснение в `kelly_dyal`.
# 🔴 КАРТАТА СПИРА ДА ГОВОРИ ЗА СЕБЕ СИ (08.09.2026, по изрично
# оплакване на собственика: «да не говори тъпотии как работи и някви
# идиотски неща и някви странни проценти»).
#
# Измерено на 38 живи карти: пет реда заемат място, без да казват нищо за
# мача — «нашата сметка казваше» (66% от картите), «залог: 1 единица» (71%,
# дума по дума един и същ), «гледани N мача» (68%, преразказва реда над
# себе си), «числото е претеглено с пазарното» (16%) и «Спортът досега:
# N познати от N» (16%).
#
# ПЪТ НАЗАД: PREDICT_KARTA_ZA_NAS=1 ги връща всичките.
KARTA_ZA_NAS = (os.environ.get("PREDICT_KARTA_ZA_NAS") or "0").strip() in (
    "1", "true", "yes", "да")

ZALOG_VKL = env_int("PREDICT_ZALOG", 1, 0, 1) == 1
# Доказаният ръб, В ЧАСТИ ОТ ЕДИНИЦАТА. Нула значи «няма доказан ръб» и
# Kelly отказва да смята. Днес е нула, защото CLV е +0.11% [−0.27 … +0.50].
KELLY_RAB = float((os.environ.get("PREDICT_KELLY_RAB") or "0").strip() or 0)
# Дробна Kelly: цялата е твърде агресивна за реален човек с реални пари.
KELLY_CHAST = float((os.environ.get("PREDICT_KELLY_CHAST") or "0.25").strip()
                    or 0.25)
KELLY_TAVAN = float((os.environ.get("PREDICT_KELLY_TAVAN") or "3").strip() or 3)


def kelly_dyal(p, cena, rab=None):
    """Колко единици да се заложат. 1.0 = равен залог.

    🔴 ОТКАЗВА БЕЗ ДОКАЗАН РЪБ. Kelly е вярна само ако `p` е истинската
    вероятност. Измерено: нашата `p` е ПО-ЛОША от пазарната (Брайер 0.2175
    срещу 0.2064), а при тегло 0.00 публикуваната `p` ВЕЧЕ Е пазарната.
    Тоест «ръбът», който би сметнала, е маржът на книгата със сменен знак —
    формулата щеше да залага ПОВЕЧЕ там, където губим повече.

    Затова без изрично подаден измерен ръб връща 1.0 (равен залог). Това не
    е предпазливост, а отказ да се смята с число, което го няма.
    """
    r = KELLY_RAB if rab is None else float(rab)
    if r <= 0.0:
        return 1.0                     # няма доказан ръб → равен залог
    try:
        p, cena = float(p), float(cena)
    except (TypeError, ValueError):
        return 1.0
    if not (0.0 < p < 1.0 and 1.0 < cena < 1000.0):
        return 1.0
    # Ръбът се СВИВА с доказания: p·c−1 е нашата оценка, а тя е доказано
    # оптимистична. `r` е делът от нея, който сме премерили, че оцелява.
    rub = (p * cena - 1.0) * r
    if rub <= 0.0:
        return 0.0                     # отрицателна стойност — не се залага
    dyal = rub / (cena - 1.0)
    return max(0.0, min(KELLY_TAVAN, dyal * KELLY_CHAST))


def _chisla_v(red):
    """Числата в един ред, като низове. Без `re` — файлът го внася по функции.

    Служи на едно място: да се разбере дали редът «📚 гледани N мача на единия
    и N на другия» само преписва числата, които 📊 вече е показал.
    """
    izlaz, tek = set(), ""
    for _z in str(red or ""):
        if _z.isdigit():
            tek += _z
        else:
            if tek:
...
def zalog_red(an):
    """«💵 залог: 1 единица (равен)» или празно.

    Казва на глас това, което ботът и без това прави. Без доказан ръб
    редът е ЕДИН И СЪЩ за всяка карта — и точно това е информацията:
    никоя карта не заслужава повече от друга, докато не се докаже.
    """
    if not ZALOG_VKL or not isinstance(an, dict):
        return ""
    c = an.get("pazar_cena")
    if not c:
        return ""                      # без цена няма и залог
    d = kelly_dyal(an.get("p"), c)
    if d <= 0.0:
        return ""
    if abs(d - 1.0) < 1e-9:
        # 🔴 ЕДНАКЪВ РЕД НЕ Е ИНФОРМАЦИЯ (08.09.2026). Измерено: 27 от 27
        # пъти дума по дума същият текст. Мястото му е в описанието на
        # стаята, не под всяка карта. РАЗЛИЧЕН залог продължава да се пише —
        # той казва нещо.
        if not KARTA_ZA_NAS:
            return ""
        return "\U0001f4b5 залог: <b>1</b> единица · равен за всяка карта"
    return ("\U0001f4b5 залог: <b>" + ("%.2f" % d).rstrip("0").rstrip(".")
            + "</b> " + mn(int(d) if d == int(d) else 2, "единица", "единици"))


def dvizhenie_red(an):
    """«📉 цената падна 1.85 → 1.68» или празно.

    🔴 САМО НАД ПРАГА НА ПОРТИЕРА. Под него е закръгляне на книгата, а ред,
    който винаги стои, спира да се чете.

    🔴 ФАКТ, БЕЗ ОБЯСНЕНИЕ. Две цени и стрелка — картите в този проект не
    изнасят лекции.

    🔴 ИСКА ДВЕ ВИДЕНИ ЦЕНИ. `dvizhenie_n ≥ 2` значи, че цената е гледана в
    две различни пускания; една точка е снимка, не движение.
    """
    if not isinstance(an, dict):
        return ""
    if abs(dvizh_tezhest(an)) <= 0.0:
        return ""                      # под прага или без редица
    sega = an.get("pazar_cena")
    predi = an.get("dvizhenie_ot")
    try:
        sega, predi = float(sega), float(predi)
    except (TypeError, ValueError):
        return ""
    if not (1.0 < sega < 1000.0 and 1.0 < predi < 1000.0):
        return ""
    if abs(sega - predi) < 0.01:
        return ""                      # еднакви до стотинка — няма какво да се каже
    strelka = "\U0001f4c9" if sega < predi else "\U0001f4c8"
    duma = "падна" if sega < predi else "се вдигна"
    return (strelka + " цената " + duma + " <b>" + ("%.2f" % predi)
            + "</b> → <b>" + ("%.2f" % sega) + "</b>")
...
def card(an, now):
    ...
    # 🔴 ВЪТРЕШНИЯТ СПОР НЕ Е ЗА ЧИТАТЕЛЯ (08.09.2026). Този блок пишеше
    # «нашата сметка казваше 54%» под заглавие «87% · много ясен фаворит» —
    # на 25 от 38 карти. Две числа за едно нещо. Старият коментар твърдеше,
    # че читателят «има право да види двете мнения»; днес е измерено, че
    # нашето число е ДОКАЗАНО по-лошо от пазарното (Brier 0.2170 срещу
    # 0.2067, разликата изцяло над нулата) — тоест показвахме му по-слабото.
    if KARTA_ZA_NAS and _pp and SG is not None \
            and str(an.get("bucket") or "") not in BEZ_SMESVANE:
        try:
    ...
    # 📉 ДВИЖЕНИЕТО НА ЦЕНАТА (05.09.2026). От днес портиерът решава с него,
    # но читателят не го виждаше. То казва накъде отиват ЧУЖДИТЕ пари, след
    # като сме казали своето — най-силният сигнал, който можем да покажем.
    # Стои ДО реда с цената, защото говори за същото число.
    _dvr = dvizhenie_red(an)
    if _dvr:
        lines.append(_dvr)

    # 💵 РАЗМЕРЪТ. Казва на глас това, което ботът и без това прави: равен
    # залог за всяка карта, докато ръб не е доказан. Виж `kelly_dyal` защо
    # формулата отказва да смята без измерено число.
    _zr = zalog_red(an)
    if _zr:
        lines.append(_zr)
    ...
    # 🔴 ИЗВАДКАТА ПАДА, КОГАТО ФОРМАТА ВЕЧЕ Е НА КАРТАТА (08.09.2026).
    # Измерено на 38 живи карти: 📚 излиза на 33, но САМО на 8 от тях има и
    # ред за формата — и точно там двата се повтарят: «20 победи и 8 загуби»
    # + «2 победи и 6 загуби», а после «гледани 8 мача на единия и 28 на
    # другия» (20+8=28, 2+6=8). На другите 25 карти 📚 казва «без история» и
    # е единственото, което говори за дълбочината — затова НЕ пада.
    _dsr = [str(r) for r in lines if str(r).startswith("\U0001f4ca ")]
    _izv = esc(an.get("sample") or "")
    # 🔴 ПРЕПИСЪТ СЕ ХВАЩА ПО ЧИСЛАТА, НЕ ПО ДУМИТЕ (08.09.2026). Първата
    # редакция търсеше «победи/загуби» и хващаше само тениса и тениса на маса;
    # футболът и бейзболът пишат формата с «вкарани и допуснати ... (11 мача)»
    # и преписваха необезпокоявани на 7 от 25 живи карти.
    #
    # Думовото правило ОСТАВА до числовото: при тениса «гледани 28» е СБОР от
    # «20 победи и 8 загуби» — числото 28 буквално го няма горе, тъй че само
    # числата не биха го уловили. Двете заедно ловят и двата почерка.
    _ch_izv = _chisla_v(_izv)
    _ch_forma = set()
    for _r in _dsr:
        _ch_forma |= _chisla_v(_r)
    _ima_forma = (any(("побед" in r or "загуб" in r) for r in _dsr)
                  or (bool(_ch_izv) and _ch_izv <= _ch_forma))
    if _ima_forma and not KARTA_ZA_NAS:
        _izv = ""
    _st = [x for x in [_izv] if x]
    # 🔴 ПРИСЪДАТА ПАДА, ФАКТЪТ ОСТАВА (08.09.2026).
    #
    # Сутринта тук пишеше «ПРЕДУПРЕЖДЕНИЕТО ОСТАВА ВИНАГИ. То пази читателя».
    # Тогава не бях мерил ДАЛИ пази. Измерено на живия дневник (1306 отсъдени
    # единични карти, изтеглен в същия ден):
    #
    #   картите, на които редът пали, се сбъдват НАД обявеното си число във
    #   всяка от четирите ленти (+1.8, +1.9, +5.9, +9.0 пункта), а
    #   непредупредените — ПОД него във всяка (−4.0, −4.5, −4.9, −2.4).
    #   Стратифицирана разлика +0.071 [+0.017 .. +0.124], извън плацебото
    #   (разбъркан етикет дава над 0.053 в 5% от случаите).
    #
    #   🔴 НО на ТОЧНАТА група (звезди ≤1 И «без история», n=93) разликата е
    #   +0.047 [−0.065 .. +0.160] — НЕ СЕ ДОКАЗВА. Тъй че твърдението е тясно:
    #   няма измерение, което да ПОДПИРА «пазете се». Не се твърди и обратното.
    #
    # Присъда без основание насочва предпазливостта в грешната посока. Живо
    # редът стоеше под «88% · много ясен фаворит» и казваше на четящия да се
    # пази точно от картата, която не се проваля.
    #
    # КАКВО БИ ГО ВЪРНАЛО: предупредените да почнат да се сбъдват ПОД
    # обявеното си число на пресни данни. Условието е живо и на едно място.
    if (KARTA_ZA_NAS and int(an.get("stars") or 0) <= 1
            and float(an.get("n_eff") or 0.0) < 10.0):
    ...
    # 🔴 НЕ СТИГА ДО ЧИТАТЕЛЯ (08.09.2026). Живо: «📈 Футболът досега:
    # 2 познати от 11» стоеше под карта, която обявява 72% — две присъди за
    # едно и също, при това втората върху 11 наблюдения. Под 30 наблюдения
    # числото е инвентар, а инвентарът е за нас.
    #
    # 🔴 ФУНКЦИЯТА НЕ СЕ ПИПА. Тя си е вярна и има 20 проверки зад себе си;
    # решението «показвам ли го» е на КАРТАТА, не в сметката.
    rec = ("" if (total or not KARTA_ZA_NAS)
...
def fallback_analyse(fx, ctx, reason):
    ...
    # 🔴 РЕЗЕРВНАТА КАРТА ОБЯСНЯВАШЕ СЕБЕ СИ (08.09.2026). Живо, на една карта:
    #     📊 Малко данни за тази среща: няма история.
    #     📊 Картата стъпва на предимството на домакина за този спорт.
    #     📚 без история · пазете се, стъпката е тънка
    # Първият казва това, което третият казва по-добре — третият и
    # ПРЕДУПРЕЖДАВА. Вторият е как е сметнат ботът, не какво става в мача.
    # (Подробността в скобите и досега не стигаше дотук: `kratko` реже на «(»,
    # тъй че първият ред винаги се изражда до «няма история».)
    if not KARTA_ZA_NAS:
        why = []
...
def _kambi_rezerva(an):
    ...
    if not (1.0 < c_dom < 1000.0 and 1.0 < c_gost < 1000.0):
        return an
    # 🔴 И РАВНИЯТ (08.09.2026). Kambi го дава като `OT_CROSS` при
    # триизходните спортове; дотук картата за равен се отказваше мълчаливо.
    try:
...
TARGUVANI = {
    ...
    # 🔴 ДОБАВЕНИ 08.09.2026 — събираха срещи и пускаха НУЛА карти.
    #
    # Здравният преглед го викаше от дни: «има срещи, няма карти — филтрите
    # изядоха всичко». Измерено живо: хокеят събира 25 срещи и портиерът
    # реже всичките 25 с «няма пазар»; ръгбито — 16 от ragbi.srechti.
    #
    # А те се предлагат от всяка книга: НХЛ, Шампионската хокейна лига и
    # финландската Лига са на bet365 и на българските къщи, същото важи за
    # Топ 14 и НПС.
    #
    # Цената идва по-късно и не бива да задържа картата: Kambi държи 42
    # хокейни събития, но нашите конкретни срещи още ги няма в прозореца му
    # (проверено — отборите се срещат, но с ДРУГИ съперници).
    #
    # «Тестови мачове» НЕ влиза: под това име минават и контроли, а те се
    # търгуват през пръсти.
    "hockey": {"НХЛ", "Шампионска хокейна лига", "СМ Лига"},
    "rugby": {"Топ 14", "НПС"},
}
...
def combo_card(idx, legs, now):
    ...
    lines = ["🎫 <b>ФИШ " + str(idx) + " НА ДЕНЯ</b> · " + date_bg(now),
             # 🔴 «4 срещи» ПАДА (08.09.2026): същото броене стои четири
             # реда по-долу като «И четирите заедно». «всички трябва да
             # познаят» остава — то казва, че един паднал крак сваля всичко.
             # n_match() НЕ се пипа: има друг читател (конзолния ред) и две
             # свои проверки.
             "<i>всички трябва да познаят</i>", ""]
    for a in legs:
        fx = a["fx"]
    ...
    lines += ["",
              ...
              # 🔴 СВЕТОФАРЪТ НА ФИША ПАДА (08.09.2026). Живо: «📊 И
              # четирите заедно: 45%», а веднага под него «🟢 стегнат фиш».
              # Зелено върху 45% е обещание, което самото число опровергава;
              # а «иска късмет в един от краката» преразказва произведението.
              # Стълбицата COMBO_DUMI остава жива и заключена от проверка.
              (combo_duma(total) if KARTA_ZA_NAS else None)] + _fc + [
              "🟢 THE GREEN ROOM"]
...
# 🔴 ЕДИНСТВЕНОТО ИЗМЕРЕНО ЧИСЛО В ПОДРЕДБАТА (05.09.2026).
#
# Дотук редът се решаваше от звездите и силата. Измерено на живия дневник
# (724 отсъдени карти с коефициент), НИТО ЕДНОТО не различава пари:
#     звезди: 1★ −4.1% · 2★ −19.2% · 3★ −9.9%
#     сила:   под 0.35 −11.8% · 0.35–0.50 −13.0% · 0.50+ +6.6% (недоказано)
#
# А нивото на коефициента различава, и държи в ЧЕТИРИ спорта поотделно:
#     коеф 1.70–2.00  ROI −24.1% [−36.9 … −11.4]  n=191
#       бейзбол −31.9% · тенис на маса −36.6% · тенис −39.1% · футбол −18.8%
#     цялата дъска −11.1% · без коеф 1.70+ → −4.4%
#
# Нула изключва правилото напълно.
KOEF_PRAG = float((os.environ.get("PREDICT_KOEF_PRAG") or "1.70").strip() or 1.70)
...
def koef_tezhest(an):
    ...
    🔴 САМО НАКАЗАНИЕ, НИКАКВО ПОВИШЕНИЕ. Измерена е ЕДНА група като
    губеща — тя слиза. Нищо не се вдига изкуствено, защото за нищо друго
    няма доказателство.

    🔴 КАРТА БЕЗ ЦЕНА НЕ СЕ НАКАЗВА. Тези карти сбъдват 67.3% при обявени
    64.6% — по-добрата половина на дъската. Липсващата цена е дупка в
    ИЗМЕРВАНЕТО, не в картата.
    """
    if KOEF_PRAG <= 0 or not isinstance(an, dict):
        return 0.0
    try:
        c = float(an.get("pazar_cena") or 0.0)
    except (TypeError, ValueError):
        return 0.0
    if c <= 1.0:
        return 0.0                    # няма цена — не се съди
    return -KOEF_SHTAFA if c >= KOEF_PRAG else 0.0


# 🔴 ЛИГА БЕЗ НИТО ЕДНА ЦЕНА (05.09.2026) — виж `ligi_bez_cena`.
BEZ_CENA_MIN = env_int("PREDICT_BEZ_CENA_MIN", 3, 0, 50)


def ligi_bez_cena(kandidati):
    """[(лига, кош, брой)] за лигите, дали карти, но НУЛА цени.

    🔴 ИЗМЕРЕНО ЗАЩО: WTT е дал 265 карти с нула цени за 33 дни, а никой не
    е видял — разбра се чак при четене на целия дневник. Точно WTT е
    най-силният ни спорт (сбъдва 67.9% при обявени 61.7%), а без цена
    картата му не може да бъде смесена с пазара, не влиза в измерването на
    парите и читателят няма как да я провери.

    Праг 3: една карта без цена е случайност, три от три е устройство.
    """
    po = {}
    for a in (kandidati or []):
        if not isinstance(a, dict):
            continue
        fx = a.get("fx") or {}
        k = (str(fx.get("league") or "?"), str(a.get("bucket") or "?"))
        ima = 1 if a.get("pazar_cena") else 0
        n, s = po.get(k, (0, 0))
        po[k] = (n + 1, s + ima)
    out = [(lg, b, n) for (lg, b), (n, s) in po.items()
           if s == 0 and n >= max(1, BEZ_CENA_MIN)]
    out.sort(key=lambda x: -x[2])
    return out


def kazhi_bez_cena(kandidati):
    """Печата лигите без нито една цена. Мълчи, когато няма такива."""
    lo = ligi_bez_cena(kandidati)
    for lg, b, n in lo:
        print("   \U0001f4b8 БЕЗ КОЕФИЦИЕНТ: " + lg[:38] + " (" + b + ") — "
              + str(n) + mn(n, " карта", " карти")
              + ", нула цени при трите книги")
    return len(lo)
...
def selftest():
    ...
    # --- 🏒🏉 ХОКЕЙ И РЪГБИ: СЪБИРАТ И ПУСКАТ (08.09.2026) ---
    #
    # Здравният преглед ги викаше от дни: «има срещи, няма карти».
    # Измерено живо: хокеят събираше 25 и портиерът режеше 25 с «няма
    # пазар»; ръгбито даваше 16 срещи и всичките падаха като «далече».
    check("хокейните лиги са в търгуваните",
          {"НХЛ", "Шампионска хокейна лига", "СМ Лига"} <= TARGUVANI.get("hockey", set()))
    check("ръгби лигите са в търгуваните",
          {"Топ 14", "НПС"} <= TARGUVANI.get("rugby", set()))
    # 🔴 «Тестови мачове» НЕ влиза — под това име минават и контроли.
    check("тестовите мачове НЕ са обявени за търгувани",
          not [x for x in TARGUVANI.get("rugby", set()) if "естов" in x])
    check("Шампионската хокейна лига минава портиера",
          ima_pazar({"bucket": "hockey",
                     "fx": {"league": "Шампионска хокейна лига"}})[0] is True)
    check("СМ Лига минава портиера",
          ima_pazar({"bucket": "hockey",
                     "fx": {"league": "СМ Лига, Финландия"}})[0] is True)
    check("Топ 14 минава портиера",
          ima_pazar({"bucket": "rugby",
                     "fx": {"league": "Топ 14, Франция"}})[0] is True)
    check("НПС минава портиера",
          ima_pazar({"bucket": "rugby",
                     "fx": {"league": "НПС, Нова Зеландия"}})[0] is True)
    check("непозната хокейна лига пак се реже",
          ima_pazar({"bucket": "hockey",
                     "fx": {"league": "Ученическа лига, Естония"}})[0] is False)
    # 🔴 ХОРИЗОНТЪТ. Ръгбито е СЕДМИЧЕН спорт: мачовете му са на 52-108 часа,
    # а общият хоризонт е 30 — при него в кофата влизаха НУЛА.
    check("ръгбито има собствен хоризонт", "rugby" in HORIZONT_PO_SPORT)
    check("и той стига до седмичния ритъм", horizont_za("rugby") >= 110)
    check("хокеят също", horizont_za("hockey") >= 110)
    check("общият хоризонт остава по-къс", HORIZON_H < horizont_za("rugby"))
    ...
    # ═══ КОЕФИЦИЕНТЪТ ТЕЖИ В ПОДРЕДБАТА (05.09.2026) ══════════════════
    #
    # 🔴 Дотук редът се решаваше от звездите и силата, а измерено на 724
    # отсъдени карти НИТО ЕДНОТО не различава пари: 1★ −4.1% · 2★ −19.2% ·
    # 3★ −9.9%; сила 0.50+ дава +6.6%, но интервалът минава през нулата.
    # Нивото на коефициента различава и държи в ЧЕТИРИ спорта поотделно:
    # коеф 1.70–2.00 → ROI −24.1% [−36.9 … −11.4], n=191.
    # Челните 9 на ден: старата подредба −13.6% [−24.7 … −2.5] 🔴,
    # новата −6.5% [−16.5 … +3.5] ⚪ — СЪЩИЯТ брой карти.
    check("скъпата карта слиза",
          koef_tezhest({"pazar_cena": 1.85}) < 0)
    check("евтината не се пипа",
          koef_tezhest({"pazar_cena": 1.45}) == 0.0)
    check("точно на прага СЛИЗА (прагът е включен)",
          koef_tezhest({"pazar_cena": KOEF_PRAG}) < 0)
    check("малко под прага не слиза",
          koef_tezhest({"pazar_cena": KOEF_PRAG - 0.01}) == 0.0)
    # 🔴 КАРТА БЕЗ ЦЕНА НЕ СЕ НАКАЗВА. Тя сбъдва 67.3% при обявени 64.6% —
    # по-добрата половина. Липсващата цена е дупка в ИЗМЕРВАНЕТО, не в
    # картата.
    check("без цена — нула", koef_tezhest({}) == 0.0)
    check("цена None — нула", koef_tezhest({"pazar_cena": None}) == 0.0)
    check("цена 1.0 (боклук) — нула", koef_tezhest({"pazar_cena": 1.0}) == 0.0)
    check("низ вместо цена не гърми", koef_tezhest({"pazar_cena": "абв"}) == 0.0)
    check("боклук вместо карта не гърми", koef_tezhest(None) == 0.0)
    check("наказанието тежи колкото ЕДНА звезда", abs(KOEF_SHTAFA - 1000.0) < 1e-9)
    # ── и ръчката го изключва напълно
    _st_kp = KOEF_PRAG
    try:
        globals()["KOEF_PRAG"] = 0.0
        check("PREDICT_KOEF_PRAG=0 изключва правилото",
              koef_tezhest({"pazar_cena": 9.9}) == 0.0)
    finally:
        globals()["KOEF_PRAG"] = _st_kp
    check("прагът е върнат", KOEF_PRAG == _st_kp)

    # 🔴 И ЧЕ СТИГА ДО САМАТА ПОДРЕДБА, не само че функцията смята.
    # Точно този шаблон ме хвана четири пъти днес: сметката работи, пътят —
    # не. Затова тук се вика `choose` и се гледа КОЙ ИЗЛИЗА.
    def _kand(ime, zv, cena):
        return {"fx": {"home": ime, "away": "Х", "when": datetime.now(SOFIA)
                       + timedelta(hours=6), "bucket": "football"},
                "bucket": "football", "pick": "1 · " + ime, "p": 0.60,
                "stars": zv, "strength": 0.2, "pazar_cena": cena}
    # 🔴 ЕДНАКВИ ЗВЕЗДИ, РАЗЛИЧНА ЦЕНА — инак сравнението е
    # двусмислено. Първата ми версия слагаше 3★ скъп срещу 2★ евтин и двете
    # излизаха с ТОЧНО еднаква тежест (2020 = 2020). Това Е намерението
    # (наказанието тежи една звезда), но тестът тогава мери равенство и го
    # обявява за провал — проверката беше сгрешена, не кодът.
    _skap = _kand("СКЪП", 2, 1.95)
    _evtin = _kand("ЕВТИН", 2, 1.30)
    # `now=None` праща всичките в общата купчина: тогава РЕДЪТ е
    # единственото, което решава. С истинско време част от тях биха минали
    # по пътя на спешните, който се сортира по ЧАС, не по тежест — и
    # проверката щеше да мери друго.
    _iz = choose([_skap, _evtin], 1)
    check("евтиният бие скъпия въпреки по-малко звезди",
          bool(_iz) and _iz[0]["fx"]["home"] == "ЕВТИН")
    # ── и обратното: изключи ли се правилото, старата подредба се връща
    try:
        globals()["KOEF_PRAG"] = 0.0
        _iz2 = choose([_skap, _evtin], 1)
        check("без правилото звездите пак решават",
              bool(_iz2) and _iz2[0]["fx"]["home"] == "СКЪП")
    finally:
        globals()["KOEF_PRAG"] = _st_kp
    # 🔴 И САМАТА МЯРКА НА НАКАЗАНИЕТО: една звезда, точно. Скъпа 3★
    # и евтина 2★ трябва да са РАВНИ по тежест — това е решението, взето с
    # числата (едно ИЗМЕРЕНО число срещу едно неизмерено), и се пази изрично,
    # за да не се промени мълчаливо при следваща настройка.
    ...
    #
    # 🔴 Портиерът решава с него от днес, но човекът отсреща виждаше само
    # крайното число. Движението казва накъде отиват ЧУЖДИТЕ пари, след
    # като сме казали своето.
    _dr_dolu = dvizhenie_red({"pazar_cena": 1.68, "dvizhenie_ot": 1.85,
                              "dvizhenie": 0.06, "dvizhenie_n": 2})
    _dr_gore = dvizhenie_red({"pazar_cena": 1.85, "dvizhenie_ot": 1.68,
                              "dvizhenie": -0.06, "dvizhenie_n": 2})
    check("падналата цена се показва", "падна" in _dr_dolu)
    check("и носи ДВЕТЕ числа",
          "1.85" in _dr_dolu and "1.68" in _dr_dolu)
    check("вдигнатата се показва като вдигната", "вдигна" in _dr_gore)
    check("стрелките са различни", _dr_dolu[:1] != _dr_gore[:1])
    # 🔴 БЕЗ ЛЕКЦИЯ. Две цени и стрелка — картите тук не обясняват.
    for _d in ("добър", "лош", "знак", "внимание", "защото"):
        check("редът не изнася лекция (%s)" % _d,
              _d not in _dr_dolu.lower() and _d not in _dr_gore.lower())
    # 🔴 ПОД ПРАГА МЪЛЧИ. Ред, който винаги стои, спира да се чете.
    check("движение под прага не се показва",
          dvizhenie_red({"pazar_cena": 1.70, "dvizhenie_ot": 1.72,
                         "dvizhenie": 0.007, "dvizhenie_n": 2}) == "")
    check("една видяна цена не рисува стрелка",
          dvizhenie_red({"pazar_cena": 1.68, "dvizhenie_ot": 1.85,
                         "dvizhenie": 0.06, "dvizhenie_n": 1}) == "")
    check("без данни — празно",
          dvizhenie_red({}) == "" and dvizhenie_red(None) == "")
    check("боклук вместо цена не гърми",
          dvizhenie_red({"pazar_cena": "абв", "dvizhenie_ot": 1.85,
                         "dvizhenie": 0.06, "dvizhenie_n": 2}) == "")
    check("еднакви до стотинка — нищо",
          dvizhenie_red({"pazar_cena": 1.850, "dvizhenie_ot": 1.851,
                         "dvizhenie": 0.06, "dvizhenie_n": 2}) == "")
    # 🔴 И ЧЕ СТИГА ДО САМАТА КАРТА, не само че функцията смята.
    # Срещата носи `emoji` — `card` го чете направо, без резерва.
    _dr_an = {"fx": {"bucket": "football", "emoji": "⚽", "home": "Аа",
                     "away": "Бб", "league": "Проба",
                     "when": datetime.now(SOFIA) + timedelta(hours=5),
                     "extra": {}},
              "bucket": "football", "pick": "1 · Аа", "p": 0.62, "stars": 2,
              "strength": 0.3, "sample": "гледани по 10 мача на всеки",
              "pazar_cena": 1.68, "dvizhenie_ot": 1.85, "dvizhenie": 0.06,
              "dvizhenie_n": 2, "why": ["едно", "две"]}
    _dr_txt = card(_dr_an, datetime.now(SOFIA))
    check("🔴 редът за движението стига до КАРТАТА", "цената падна" in _dr_txt)
    check("и не изхвърля реда с цената", "1.68" in _dr_txt)
    check("картата остава чиста от забранени думи",
          banned_word(_dr_txt) is None)

    # ═══ 💵 РАЗМЕРЪТ НА ЗАЛОГА (05.09.2026) ═══════════════════════════
    #
    # 🔴 KELLY ОТКАЗВА БЕЗ ДОКАЗАН РЪБ. Тя е вярна само ако `p` е
    # истинската вероятност; измерено — нашата е ПО-ЛОША от пазарната
    # (Брайер 0.2175 срещу 0.2064), а при тегло 0.00 публикуваната `p`
    # ВЕЧЕ Е пазарната. «Ръбът», който би сметнала, е маржът на книгата със
    # сменен знак: формулата щеше да залага ПОВЕЧЕ там, където губим повече.
    check("без доказан ръб — равен залог",
          abs(kelly_dyal(0.62, 1.75) - 1.0) < 1e-9
          and abs(kelly_dyal(0.85, 1.20) - 1.0) < 1e-9)
    check("и ръбът по подразбиране Е нула (днес не е доказан)",
          KELLY_RAB == 0.0)
    # ── а с подаден ръб смята и отказва при отрицателна стойност
    check("с доказан ръб дава дял", 0.0 < kelly_dyal(0.62, 1.75, 0.30) < 1.0)
    check("🔴 при отрицателна стойност НЕ се залага",
          kelly_dyal(0.50, 1.80, 0.30) == 0.0)
    check("по-голям ръб дава по-голям дял",
          kelly_dyal(0.62, 1.75, 0.60) > kelly_dyal(0.62, 1.75, 0.30))
    # 🔴 ТАВАНЪТ СЕ ИЗПИТВА ТАМ, КЪДЕТО МОЖЕ ДА УХАПЕ. Дялът на Kelly е
    # най-много 1, а дробната част е 0.25 — тоест при днешните стойности
    # изходът не може да мине 0.25 и таван 3 НИКОГА не се задейства.
    # Първата ми проверка го „пазеше" с числа, които не го докосват:
    # мутация, махаща тавана, оцеля. Тук частта се вдига нарочно.
    _st_kch = KELLY_CHAST
    try:
        globals()["KELLY_CHAST"] = 20.0
        check("таванът реже, когато частта е голяма",
              abs(kelly_dyal(0.99, 50.0, 1.0) - KELLY_TAVAN) < 1e-9)
    finally:
        globals()["KELLY_CHAST"] = _st_kch
    check("частта е върната", KELLY_CHAST == _st_kch)
    check("при днешните стойности дялът не мине частта",
          kelly_dyal(0.99, 50.0, 1.0) <= KELLY_CHAST + 1e-9)
    check("дробната част е под единица (цялата Kelly е агресивна)",
          0.0 < KELLY_CHAST <= 1.0)
    check("боклук дава равен залог",
          abs(kelly_dyal(None, None, 0.3) - 1.0) < 1e-9
          and abs(kelly_dyal("абв", 1.8, 0.3) - 1.0) < 1e-9
          and abs(kelly_dyal(1.5, 1.8, 0.3) - 1.0) < 1e-9)
    # ── редът на картата
    # 🔴 ОБЪРНАТА (08.09.2026). Измерено на 38 живи карти: този ред излиза
    # 27 пъти ДУМА ПО ДУМА един и същ. Ред, който не се мени, не носи
    # информация — мястото му е в описанието на стаята.
    _zr1 = zalog_red({"p": 0.62, "pazar_cena": 1.75})
    check("равният залог вече НЕ се пише на всяка карта", _zr1 == "")
    _st_kzn = globals().get("KARTA_ZA_NAS")
    try:
        globals()["KARTA_ZA_NAS"] = True
        _zr_nazad = zalog_red({"p": 0.62, "pazar_cena": 1.75})
        check("но пътят назад го връща", "залог" in _zr_nazad)
        check("и той пак казва «равен»", "равен" in _zr_nazad)
    finally:
        globals()["KARTA_ZA_NAS"] = _st_kzn
    check("ръчката за връщане е върната", KARTA_ZA_NAS is _st_kzn)
    check("без цена няма ред", zalog_red({"p": 0.62}) == "")
    check("боклук не гърми", zalog_red(None) == "")
    _st_zv = ZALOG_VKL
    try:
        globals()["ZALOG_VKL"] = False
        check("PREDICT_ZALOG=0 маха реда",
              zalog_red({"p": 0.62, "pazar_cena": 1.75}) == "")
    finally:
        globals()["ZALOG_VKL"] = _st_zv
    check("ръчката е върната", ZALOG_VKL is _st_zv)
    # 🔴 И ЧЕ НЕ СТИГА ДО КАРТАТА — но се връща с ръчката.
    check("равният залог НЕ стига до картата",
          "залог" not in card(dict(_dr_an), datetime.now(SOFIA)))
    _st_kz2 = globals().get("KARTA_ZA_NAS")
    try:
        globals()["KARTA_ZA_NAS"] = True
        check("а с PREDICT_KARTA_ZA_NAS=1 стига",
              "залог" in card(dict(_dr_an), datetime.now(SOFIA)))
    finally:
        globals()["KARTA_ZA_NAS"] = _st_kz2
    ...
    #
    # 🔴 WTT е дал 265 карти с НУЛА цени за 33 дни и никой не е видял —
    # разбра се чак при четене на целия дневник. И точно WTT е най-силният
    # ни спорт: сбъдва 67.9% при обявени 61.7%, докато дребните лиги, които
    # ИМАТ цени, сбъдват 52.2%. Без цена картата не се смесва с пазара, не
    # влиза в измерването на парите и читателят не може да я провери.
    def _bc(lg, cena, n):
        return [{"fx": {"league": lg}, "bucket": "tabletennis",
                 "pazar_cena": cena} for _ in range(n)]
    _bc_vsi = _bc("БЕЗ ЦЕНА", None, 5) + _bc("С ЦЕНА", 1.75, 4)         + _bc("ЕДНА САМО", None, 1) + _bc("ЧАСТИЧНО", None, 3)         + _bc("ЧАСТИЧНО", 1.60, 1)
    _bc_lo = {x[0] for x in ligi_bez_cena(_bc_vsi)}
    check("лига без нито една цена се обявява", "БЕЗ ЦЕНА" in _bc_lo)
    check("лига с цени не се обявява", "С ЦЕНА" not in _bc_lo)
    # 🔴 ЕДНА КАРТА Е СЛУЧАЙНОСТ, ТРИ ОТ ТРИ Е УСТРОЙСТВО.
    check("под прага не се обявява", "ЕДНА САМО" not in _bc_lo)
    # 🔴 И ЧАСТИЧНОТО НЕ Е ЛИПСА: три без цена и една с цена значи, че
    # пътят работи и просто не всеки мач е качен при книгата.
    check("частично покритие НЕ е липса", "ЧАСТИЧНО" not in _bc_lo)
    check("боклук не гърми",
          ligi_bez_cena(None) == [] and ligi_bez_cena([1, "х"]) == [])
    check("подредено по големина",
          [x[0] for x in ligi_bez_cena(_bc("А", None, 9) + _bc("Б", None, 4))]
          == ["А", "Б"])
    ...
    try:
        ...
        # 🔴 ОБЪРНАТА (08.09.2026). Пазеното твърдение е същото — обяснението
        # за СТАРАТА страна не бива да остане, след като изборът е обърнат.
        # Но и новото не стига до читателя: «числото е претеглено с пазарното»
        # е факт за нас, не за мача. Затова списъкът е ПРАЗЕН.
        check("обяснението за старата страна е махнато", _ob.get("why") == [])
        _st_kz8 = globals().get("KARTA_ZA_NAS")
        try:
            globals()["KARTA_ZA_NAS"] = True
            globals()["_pazar_surovo"] = _obraten
            _ob2 = dobavi_pazar({"fx": {"home": "Аа", "away": "Бб"},
                                 "bucket": "tabletennis", "pick": "1 · Аа",
                                 "p": 0.62})
            check("пътят назад връща вътрешното обяснение",
                  _ob2.get("why") == ["числото е претеглено с пазарното"])
            check("и то пак е САМО за новата страна", len(_ob2.get("why")) == 1)
        finally:
            globals()["KARTA_ZA_NAS"] = _st_kz8
    ...
    # 🔴 И РЕДЪТ ЗА ЛИПСВАЩАТА ЦЕНА — по същата причина и със същата
    # слабост: `run()` праща в Telegram и не може да бъде пуснат тук.
    _i_bc = _src_run.find("kazhi_bez_cena(")
    check("`run` казва кои лиги са без цена", _i_bc > 0)
    check("и го прави ПРЕДИ записа на тефтера", 0 < _i_bc < _i_per)
    check("низът не пази сам себе си", _src_run.count("kazhi_bez_cena(") == 1)
    ...
    # 🔴 ТЕЗИ ДВЕ СЧУПИХА РУТЕРА ЗА ДВА ДНИ (поправено 05.09.2026).
    #
    # Те искат post_predict да върне УСПЕХ. А успех се връща само при
    # DRY_RUN или при наличен токен:
    #     if DRY_RUN: return True
    #     if not CHAT_ID or not BOT_TOKEN: return otkaz("bez_token")
    #
    # Стъпките «Самопроверка на будилника и предсказателя» (router.yml) и
    # «Портиерът на предсказателя» (support.yml) НЯМАТ env блок — нито
    # PREDICT_DRY_RUN, нито токен. Локално аз винаги подавам сухия режим и
    # затова не съм виждал провала. Дневникът на Actions го казва дословно:
    #     САМОПРОВЕРКА: 1429 наред, 2 счупени
    # и рутерът пада НАПЪЛНО: 400 ръна, нула успешни, от 03.09.
    #
    # Лекът НЕ е ред в yml-а — тогава следващият workflow пак ще падне.
    # Проверка, минаваща само при определено стартиране, мери стартирането,
    # не кода. Затова сухият режим се слага ТУК и се връща в finally.
    _dr_predi = globals()["DRY_RUN"]
    try:
        ...
        # 🔴 И ЗАБРАНАТА ПАК ВАЖИ В СУХ РЕЖИМ. Инак горните две можеха да
        # минават просто защото вратата е отворена за всичко.
        check("сухият режим НЕ отваря вратата за забранена дума",
              post_predict("букмейкър дава 1.85", PREDICT_THREAD) is False)
        check("и чужда стая пак се отказва",
              post_predict("коефициент 1.85", "26") is False)
    finally:
        globals()["DRY_RUN"] = _dr_predi
    # 🔴 И МЕТА-ПРОВЕРКА: вратата навън НЕ БИВА да зависи от това как е
    # пуснат тестът. Без токен и без сух режим тя трябва да ОТКАЗВА — а
    # проверките горе вече не разчитат на това.
    check("без сух режим и без токен вратата отказва",
          (post_predict("коефициент 1.85", PREDICT_THREAD) is False)
          if not (DRY_RUN or (CHAT_ID and BOT_TOKEN)) else True)
    ...
    # 🔴 ДВОЙКАТА ЗА РЕЗЕРВНАТА КАРТА (08.09.2026). Тя вече не обяснява себе
    # си на читателя — «Малко данни за тази среща: няма история» повтаряше
    # «📚 без история», а «Картата стъпва на предимството на домакина» е как
    # работи ботът. Пази се И ДВЕТЕ ПОСОКИ: че падат при подразбирането и че
    # ръчката ги връща. Без втората половина обезсилена ръчка би мълчала.
    check("резервната карта не обяснява себе си", _ba["why"] == [])
    check("и това важи за всеки спорт",
          fallback_analyse({"bucket": "tennis", "emoji": "🎾", "home": "А",
                            "away": "Б", "league": "Т", "extra": {}},
                           {}, "няма история")["why"] == [])
    _st_kzb = globals().get("KARTA_ZA_NAS")
    try:
        globals()["KARTA_ZA_NAS"] = True
        _ba_n = fallback_analyse({"bucket": "basketball", "emoji": "🏀",
                                  "home": "Никс", "away": "Хийт",
                                  "league": "Тест", "extra": {}},
                                 {}, "няма история")
        check("пътят назад връща обясненията", len(_ba_n["why"]) == 2)
        check("и първото пак казва колко са данните",
              "Малко данни" in _ba_n["why"][0])
        _ti_n = fallback_analyse({"bucket": "tennis", "emoji": "🎾", "home": "А",
                                  "away": "Б", "league": "Т", "extra": {}},
                                 {}, "няма история")
        check("а при личните спортове второто е за двойката",
              "Двойката е неутрална" in _ti_n["why"][1])
    finally:
        globals()["KARTA_ZA_NAS"] = _st_kzb
    ...
    # 🔴 ОБЪРНАТИ (08.09.2026). Присъдата не стига до читателя — измерено е,
    # че няма измерение, което да я подпира (виж бележката при самия ред).
    # УСЛОВИЕТО обаче е живо и непокътнато, тъй че се мери при пусната ръчка:
    # прагът 10, звездите, границата — всичко пази същото както преди.
    check("присъдата за тънката стъпка вече не стига до картата",
          "стъпката е тънка" not in _karta(1, 4.0, samp(3, 3)))
    _st_kzp = globals().get("KARTA_ZA_NAS")
    try:
        globals()["KARTA_ZA_NAS"] = True
        ...
        globals()["KARTA_ZA_NAS"] = _st_kzp
    check("ръчката е върната", KARTA_ZA_NAS is _st_kzp)
    ...
    # 🔴 ОБЪРНАТА (08.09.2026). Редът е ПРЕПИС: числото 82 стои и в двата
    # реда 📊 над него. Пази се обаче ПО-СИЛНОТО твърдение — че липсва точно
    # защото е препис, а не защото някой го е махнал.
    check("редът за данните е препис на числата горе",
          _chisla_v("гледани по 82 мача на всеки")
          <= set().union(*[_chisla_v(_r) for _r in txt.split(NL)
                           if _r.startswith("📊 ")] or [set()]))
    check("и затова вече не се повтаря долу",
          "гледани по 82 мача на всеки" not in txt)
    # (Пътят назад за този ред се мери в цикъла по всички демота по-долу —
    # тук картата идва от чужд, вече приключил стенд и не се пресмята пак.)
    ...
    # 🔴 ОБЪРНАТИ (08.09.2026). Този ред излизаше на 25 от 38 живи карти и
    # показваше ВТОРО число за същото нещо: заглавие «87% · много ясен
    # фаворит», а два реда по-долу «нашата сметка казваше 54%». Читателят
    # получаваше два отговора и нито един ориентир кой е верният.
    # И днес е измерено, че нашето число е ДОКАЗАНО по-лошото: Brier 0.2170
    # срещу пазарни 0.2067, разликата +0.0103 [+0.0059..+0.0149] изцяло над
    # нулата. Тоест показвахме по-слабото си число до по-силното.
    check("картата НЕ показва вътрешния ни спор",
          "нашата сметка казваше" not in _t_v)
    check("и не носи второ число за същото",
          "по-смели" not in _t_v and "по-уверен" not in _t_v)
    _st_kz3 = globals().get("KARTA_ZA_NAS")
    try:
        globals()["KARTA_ZA_NAS"] = True
        _t_nazad = card(_an_v, now)
        check("пътят назад връща и този ред",
              "нашата сметка казваше" in _t_nazad)
        check("и той пак е в проценти", "62%" in _t_nazad)
    finally:
        globals()["KARTA_ZA_NAS"] = _st_kz3
    ...
    # 🔴 ОБЪРНАТИ. «пазарът мисли същото» е вярно по конструкция и празно
    # като смисъл — публикуваното число ВЕЧЕ Е пазарното.
    _an_edn = dict(_an_v, p=0.55)
    check("«пазарът мисли същото» вече не се пише",
          "пазарът мисли същото" not in card(_an_edn, now))
    _an_pl = dict(_an_v, p=0.40)
    check("нито «пазарът е по-уверен»",
          "по-уверен" not in card(_an_pl, now))
    _st_kz4 = globals().get("KARTA_ZA_NAS")
    try:
        globals()["KARTA_ZA_NAS"] = True
        check("пътят назад връща и двата",
              "пазарът мисли същото" in card(_an_edn, now)
              and "по-уверен" in card(_an_pl, now))
    finally:
        globals()["KARTA_ZA_NAS"] = _st_kz4
    # 🔴 И НАЙ-ВАЖНОТО: КАРТАТА ВЕЧЕ НЕ СИ ПРОТИВОРЕЧИ. Заглавният процент
    # е единственият процент за прогнозата.
    _t_bez = card(_an_pl, now)
    check("на картата има ЕДИН процент за прогнозата",
          _t_bez.count("%</b>") <= 1)
    ...
    # 🔴 СВИДЕТЕЛЯТ НА ЧИСЛОВОТО ПРАВИЛО (08.09.2026). Футболното демо е
    # единственото, чийто препис се лови САМО по числата: редовете 📊 пишат
    # «вкарани и допуснати гола за мач (76 мача)» — думата «победи» я няма,
    # тъй че думовото правило мълчи. Изключи ли някой числовото, ТУК гърми.
    check("футболните 📊 нямат думата победи/загуби",
          all(("побед" not in _r and "загуб" not in _r)
              for _r in txt_f.split(NL) if _r.startswith("📊 ")))
    check("числата на извадката ги има горе",
          _chisla_v(demo_f["sample"])
          <= set().union(*[_chisla_v(_r) for _r in txt_f.split(NL)
                           if _r.startswith("📊 ")] or [set()]))
    check("и затова извадката НЕ се повтаря долу",
          NL + "📚 " not in txt_f)
    # Отрицателна контрола: сменят ли се числата ѝ с такива, които ГИ НЯМА
    # горе, редът се връща. Инак горното твърдение би минавало и при пазач,
    # който просто маха 📚 винаги.
    _demo_fr = dict(demo_f, sample=samp(9, 7))
    check("но с ЧУЖДИ числа извадката се връща",
          NL + "📚 " in card(_demo_fr, now))
    ...
    # 🔴 СВИДЕТЕЛЯТ НА ДУМОВОТО ПРАВИЛО (08.09.2026). При тениса и тениса на
    # маса числото в 📚 е СБОР: «20 победи и 8 загуби» -> «гледани 28 мача».
    # 28 буквално го НЯМА горе, тъй че числовото правило мълчи и преписът се
    # лови само по думата. Без този свидетел изключването на думовото правило
    # минаваше с «0 счупени» — мутацията го показа.
    _demo_du = dict(demo_t,
                    why=["Дан Цю: 20 победи и 8 загуби за последните 14 дни"],
                    sample=samp(28, 8))
    _t_du = card(_demo_du, now)
    _ch_gore = set().union(*[_chisla_v(_r) for _r in _t_du.split(NL)
                             if _r.startswith("📊 ")] or [set()])
    check("сборното число го НЯМА в редовете 📊",
          not (_chisla_v(_demo_du["sample"]) <= _ch_gore))
    check("но думовото правило пак разпознава преписа",
          NL + "📚 " not in _t_du)
    # Отрицателна контрола: същите числа, но БЕЗ думата «победи» горе.
    # Тогава нито едното правило не бива да сработи и редът се връща.
    _demo_bd = dict(demo_t, why=["Дан Цю: серия от 3 поредни мача"],
                    sample=samp(28, 8))
    check("без думата преписът не се разпознава",
          NL + "📚 " in card(_demo_bd, now))
    ...
    try:
        ...
        # 🔴 СВАЛЕН ТАВАН И ПОД (08.09.2026). Отпаднаха пет реда, които
        # говореха за бота, а не за мача — картата слезе на седем. Подът
        # ОСТАВА пазен и е смислен: час+лига, имена, празен ред, прогноза,
        # процент и поне една причина са шест; под шест картата е осакатена.
        check("новата карта е между шест и единайсет реда", 6 <= len(_nr) <= 11)
        check("и има поне една причина за мача", _nk.count(NL + "📊 ") >= 1)
        ...
        # 🔴 МЕРИ СЕ ВЪРХУ ПРИСЪСТВАЩИТЕ (08.09.2026). Дотук изброяваше 📊 и
        # 📚 поименно и гърмеше с ValueError, щом един от двата отпадне —
        # а футболното демо е точно случаят, който днес се лекува: «(76 мача)»
        # и «(74 мача)» горе, «гледани 76 ... и 74 ...» долу. Твърдението е
        # същото: прогнозата стои ПРЕДИ всеки друг означен ред.
        _dr_em = [_e for _e in ("📊", "📚", "💰", "📈", "💵", "📐")
                  if _e in _nk]
        check("има какво да се подрежда след прогнозата", len(_dr_em) >= 1)
        check("прогнозата се вижда преди всичко друго",
              all(_nk.index("🎯") < _nk.index(_e) for _e in _dr_em))
        ...
        # 🔴 ОБЪРНАТИ (08.09.2026). Редът «📈 Спортът досега: N познати от N»
        # вече не стига до читателя: живо той изглеждаше «📈 Футболът досега:
        # 2 познати от 11» под карта, която обявява 72% — две присъди за едно
        # и също, при това втората върху 11 наблюдения. Инвентарът е за нас.
        #
        # Твърденията за ФОРМАТА му се пазят, но се мерят при ПУСНАТА ръчка:
        # върне ли някой реда утре, формата му пак е пазена.
        _rec = [r for r in _nr if r.startswith("📈")]
        check("сметката на бота вече НЕ е на картата", len(_rec) == 0)
        check("на картата има точно един процент", _nk.count("%") == 1)
        _st_kz5 = globals().get("KARTA_ZA_NAS")
        try:
            globals()["KARTA_ZA_NAS"] = True
            _record_cache.clear()
            _nk2 = card(demo_f, now)
            _rec2 = [r for r in _nk2.split(NL) if r.startswith("📈")]
            check("пътят назад я връща", len(_rec2) == 1)
            check("сметката на бота е БЕЗ процент", "%" not in _rec2[0])
            check("сметката на бота носи етикет на числителя",
                  "познати от" in _rec2[0])
            check("сметката на бота брои целия живот",
                  "4 познати от 12" in _rec2[0])
        finally:
            globals()["KARTA_ZA_NAS"] = _st_kz5
            _record_cache.clear()

        # --- ПРАГЪТ RECORD_MIN още работи, само вече не пази процент, а извод.
        _kr_dnevnik(2, 1)
        _mal = card(demo_f, now)
        check("под прага сметката СЪЩО не се вижда",
              "2 познати от 3" not in _mal)
        _st_kz6 = globals().get("KARTA_ZA_NAS")
        try:
            globals()["KARTA_ZA_NAS"] = True
            _record_cache.clear()
            _mal2 = card(demo_f, now)
            check("с ръчката под прага сметката се вижда",
                  "2 познати от 3" in _mal2)
            check("под прага се казва, че е рано",
                  "още е рано за извод" in _mal2)
        finally:
            globals()["KARTA_ZA_NAS"] = _st_kz6
            _record_cache.clear()
        _mal = card(demo_f, now)
        ...
        # --- 🔴 ОБЪРНАТИ (08.09.2026). Опасението в старата бележка беше
        # вярно — «махането му не се вижда никъде» — и точно затова УСЛОВИЕТО
        # не е пипано: маха се само печатът, а тези проверки продължават да
        # пазят и прага, и мястото до 📚, при пусната ръчка.
        _tanko = dict(demo_f)
        _tanko["stars"], _tanko["n_eff"], _tanko["sample"] = 1, 6.0, samp(4, 3)
        _tt = card(_tanko, now)
        check("тънката извадка вече не носи присъда",
              "стъпката е тънка" not in _tt)
        _st_kzt = globals().get("KARTA_ZA_NAS")
        try:
            globals()["KARTA_ZA_NAS"] = True
            _tt2 = card(_tanko, now)
            check("тънката извадка носи предупреждение",
                  "стъпката е тънка" in _tt2)
            check("предупреждението е при извадката, не само",
                  any("📚" in r and "стъпката е тънка" in r
                      for r in _tt2.split(NL)))
        finally:
            globals()["KARTA_ZA_NAS"] = _st_kzt
    ...
    for dm in (demo, demo_f, demo_v, demo_h, demo_t, demo_x):
        ...
        # 🔴 ОБЪРНАТА (08.09.2026) и ЗАСИЛЕНА. Дотук: «📚 е на всяка карта».
        # От днес редът пада там, където 📊 вече е показал същите числа.
        # Новото твърдение е по-строго от старото: липсва ли 📚 на карта,
        # чиито числа ги НЯМА горе, това е дефект и пазачът гърми.
        _dsr_t = [_r for _r in t.split(NL) if _r.startswith("📊 ")]
        _ch_i = _chisla_v(str(dm.get("sample") or ""))
        _ch_f = set()
        for _r in _dsr_t:
            _ch_f |= _chisla_v(_r)
        _prepis = (any(("побед" in _r or "загуб" in _r) for _r in _dsr_t)
                   or (bool(_ch_i) and _ch_i <= _ch_f))
        check("карта " + tag + ": извадката липсва САМО когато е препис",
              (NL + "📚 " in t) or _prepis)
        _st_kza = globals().get("KARTA_ZA_NAS")
        try:
            globals()["KARTA_ZA_NAS"] = True
            check("карта " + tag + ": пътят назад връща суровата извадка",
                  NL + "📚 " in card(dm, now))
        finally:
            globals()["KARTA_ZA_NAS"] = _st_kza
    ...
    # 🔴 ОБЪРНАТА (08.09.2026). Пазеше, че светофарът е на фиша. Сега пази
    # ДВЕТЕ посоки: че го няма при подразбирането и че ръчката го връща —
    # инак обезсилена ръчка би минала мълчешком.
    check("фишът вече НЕ носи светофар",
          not any(d in cc for _p, d in COMBO_DUMI))
    check("и не носи втора зелена точка над подписа",
          cc.count("🟢") == 1 and cc.rstrip().endswith("THE GREEN ROOM"))
    _st_kzf = globals().get("KARTA_ZA_NAS")
    try:
        globals()["KARTA_ZA_NAS"] = True
        _cc_n = combo_card(1, legs5, now)
        check("пътят назад връща присъдата с думи",
              any(d in _cc_n for _p, d in COMBO_DUMI))
        check("и стълбицата съди по числото, не по броя крака",
              combo_duma(0.50).startswith("🟢")
              and combo_duma(0.25).startswith("🟡")
              and combo_duma(0.05).startswith("🟠"))
    finally:
        globals()["KARTA_ZA_NAS"] = _st_kzf
    # 🔴 БРОЕНЕТО В ЗАГЛАВИЕТО ПАДА, ДУМИТЕ ОСТАВАТ.
    check("фишът не брои краката два пъти",
          "срещи · всички трябва" not in cc and "мача · всички трябва" not in cc)
    check("но пак казва колко са заедно", "И петте заедно" in cc)
    ...
    try:
        ...
        # 🔴 РЕДЪТ ВЕЧЕ НЕ СТИГА ДО ЧИТАТЕЛЯ (08.09.2026) — виж `card`.
        # Но правилото «CS2 брои САМО своите 12» ОСТАВА пазено: то се мери
        # при пусната ръчка, за да не се загуби знанието.
        check("картата вече НЕ носи сметката на бота",
              "познати от" not in _k_cs)
        _st_kz7 = globals().get("KARTA_ZA_NAS")
        try:
            globals()["KARTA_ZA_NAS"] = True
            _record_cache.clear()
            _k_cs2 = card({"fx": {"emoji": "🎮", "home": "FURIA",
                                  "away": "paiN", "league": "CS2 · BLAST",
                                  "when": _tn_es + timedelta(hours=4),
                                  "extra": {"igra": "cs2"}},
                           "bucket": "esports", "pick": "1 · победа FURIA",
                           "p": 0.62, "stars": 1, "sample": "цената на пазара",
                           "n_eff": 0.0, "why": ["FURIA: цена 1.18"]}, _tn_es)
            check("с ръчката картата на CS2 носи сметката НА CS2",
                  "2 познати от 12" in _k_cs2)
            check("и НЕ носи слятото 24", "от 24" not in _k_cs2)
        finally:
            globals()["KARTA_ZA_NAS"] = _st_kz7
            _record_cache.clear()
    finally:

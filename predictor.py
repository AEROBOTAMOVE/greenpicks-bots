...
def post_combos(picks, cands, state, now):
    ...
    for i in range(COMBO_COUNT):
        ...
        if post_predict(combo_card(i + 1, legs, now), PICKS_THREAD):
            ...
            # 🔴 ЗАПИСВАМЕ СЛЕД ВСЕКИ ФИШ, НЕ СЛЕД ТРИТЕ (12.08.2026).
            # Отбелязването стоеше чак под цикъла. Убит рън по средата — а
            # рутерът има таван от 25 минути — оставяше „ГГГГ-ММ-ДД|combos"
            # неотбелязан и следващото пускане пращаше И ТРИТЕ фиша наново.
            # Фиш 1 излизаше два пъти в стая 4, а log_pick дедуплира, тоест
            # оценителят не гъкваше: дубълът се виждаше само в стаята.

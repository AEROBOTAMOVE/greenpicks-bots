...
BANNED_TOKENS = ["bet365", "pinnacle", "bwin", "efbet", "winbet", "palmsbet",
                 ...
                 "коефициент", "букмейкър", "odds", "залагай", "заложи"]
...
def save_state(state, now):
    """Пазим само последните три дни — файлът не бива да расте вечно."""
    keep = set()
    for i in range(0, 3):
...
def match_key(fx, now):
    """Ключ на срещата за деня. Стабилен през източниците.
    ...
    return (now.strftime("%Y-%m-%d") + "|" + str(fx.get("bucket"))
            + (("|" + tag) if tag else "")
...
def analyse(fx, ctx):
    ...
    if b == "football":
        ...
        if p1 >= px and p1 >= p2:
            ...
            if p1 + px >= p2 + px:
                pick, p = fx["home"] + " не губи", p1 + px
            else:
                pick, p = fx["away"] + " не губи", p2 + px
        strength = strength_1x2(p1, px, p2)
        if strength_binary(m["p_over"]) >= 0.24 and strength_binary(m["p_over"]) > strength_binary(m["p_btts"]):
            second = (("Над 2.5 гола: <b>" + pct(m["p_over"]) + "</b>")
                      if m["p_over"] >= 0.5
                      else ("Под 2.5 гола: <b>" + pct(1.0 - m["p_over"]) + "</b>"))
        elif strength_binary(m["p_btts"]) >= 0.24:
            second = (("И двата бележат: <b>" + pct(m["p_btts"]) + "</b>")
                      if m["p_btts"] >= 0.5
                      else ("Няма гол от единия: <b>" + pct(1.0 - m["p_btts"]) + "</b>"))
        ...
        pick = fx["home"] if fav_home else fx["away"]
        p = m["p_home"] if fav_home else m["p_away"]
        strength = strength_binary(m["p_home"])
        second = ("Очаквани точки " + one(m["exp_h"], 0) + " : " + one(m["exp_a"], 0)
                  + " (общо " + one(m["total"], 0) + ")")
        ...
        second = ("Най-вероятен резултат: <b>" + str(best[0]) + "-" + str(best[1])
                  + "</b> (" + pct(best[2] / max(1e-9, p)) + " от победите)")
...
def header_card(now, count, seen):
    return (chr(129504) + " <b>БОТА ПРЕДРИЧА</b> · " + date_bg(now) + NL
            + "Днес: <b>" + str(count) + "</b> прогнози от " + n_match(seen)
            + " под лупата.")
...
def run():
    ...
    for fx in pool:
        ...
        if already_posted(state, k):
            print("   ⏭ вече е пусната днес: " + str(fx.get("home")) + " - " + str(fx.get("away")))
            continue
        if k in seen_keys:
            # Бейзболът играе по два мача в един ден срещу същия съперник.
            # Един ключ = една карта, иначе стаята получава близнаци.
    ...
    seen = len(fresh)
    if not cands:
        if not (state.get("posted") or {}):
            post_predict(nothing_card(now, seen, thin, weak))
        else:
            print("Нищо ново убедително — мълча (днес вече има пуснати карти).")
    ...
    if not already_posted(state, fkey):
...
def selftest():
    ...
    # --- картата: кратка, чиста, с прогноза
    demo = {"fx": {"bucket": "basketball", "emoji": "🏀", "home": "Ню Йорк Никс",
                   "away": "Маями Хийт", "league": "НБА", "when": None, "time": "21:30"},
            "bucket": "basketball", "pick": "Ню Йорк Никс", "p": 0.68,
            "second": "Очаквани точки 112 : 105 (общо 217)",
    ...
    check("картата носи прогнозата", "ПРОГНОЗА: Ню Йорк Никс" in txt and "68%" in txt)
    check("картата носи звезди и извадка", "⭐⭐" in txt and "извадка 82+82" in txt)
    # 18+ стои САМО в закачените текстове (канал и стаи), не на всяка карта.
    check("картата е без предупредителен ред", "18+" not in txt and "гаранция" not in txt)
    check("картата не поучава читателя",
          not any(w in txt.lower() for w in ("отговорно", "решението е твое", "банка")))
    check("картата има най-много две обяснения", txt.count(NL + "• ") == 2)
    check("картата е чиста от хазартни думи", banned_word(txt) is None)
    check("заглавната карта е кратка", len(header_card(now, 3, 14)) < 200)
    check("подписът е чист", banned_word(footer_card(14, 3, 2, 8)) is None)
    check("картата за мълчание е чиста", banned_word(nothing_card(now, 9, 5, 4)) is None)

...
def sport_record(bucket):
    ...
    try:
        ...
        if rows or True:
            ...
            if n >= RECORD_MIN:
                red = ("📈 " + ime + " досега: <b>" + str(p) + " познати от "
                       + str(n) + "</b>")
            elif n > 0:
                red = ("📈 " + ime + " досега: " + str(p) + " познати от "
...
def post_totali(picks, state, now):
    ...
    for nov in gotovi[:max(0, int(mesto))]:
        ...
        izlezli += 1
        karti += 1
        staya = SPORT_ROOM.get(nov.get("bucket"))
        if staya and staya != PREDICT_THREAD:
            time.sleep(1.2)
            if post_predict(txt, staya):
                izlezli += 1
        time.sleep(SEND_GAP)
...
def selftest():
    ...
    check("едно пускане дава до девет неспешни", MAX_PICKS == 9)
    check("денят побира шейсет", MAX_DAY == 60)

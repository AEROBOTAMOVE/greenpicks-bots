...
def footer_card(seen, thin, weak, sports):
    ...
    return NL.join([
        ...
        "Звездите са за ДАННИТЕ зад картата. Колко е сигурен изборът пише"
        " с думи до самия процент.",
        "🟢 THE GREEN ROOM",
    ])
...
RUN_HOURS = (8, 10, 12, 14, 16, 18, 20, 22)
...
def run():
    ...
    for fx in fresh:
        ...
        if float(an.get("p") or 0.0) < MIN_SHOW_P:
            pod_praga.append(an)
            weak += 1
            print("   ✖ " + name + ": " + pct(an["p"]) + " — под прага "
                  + pct(MIN_SHOW_P) + ", не излиза")
            continue
        cands.append(an)
        st = an["stars"]
    ...
    if pod_praga:
        ...
        print("   Под прага " + pct(MIN_SHOW_P) + ": " + str(len(pod_praga))
...
def selftest():
    ...
    check("51% минава прага", 0.51 >= MIN_SHOW_P)
    ...
    check("подписът пази легендата на звездите", "⭐⭐⭐" in _podpis)
    check("подписът е кратък", len(_podpis) < 260)
    ...
    check("в 20:05 следващото пускане е в 22:00",
          next_run(datetime(2026, 8, 5, 20, 5, tzinfo=SOFIA)).hour == 22)
    check("в 10:05 следващото пускане е в 12:00", next_run(_v10).hour == 12)
    check("часовете на пускане са осем", len(RUN_HOURS) == 8)
    # Последният е 22, не 23: кронът е ПЛАНЪТ, а GitHub закъснява със средно
    # 65 минути (измерено върху 55 пускания). Планиран 22:00 значи реален 23:0x.

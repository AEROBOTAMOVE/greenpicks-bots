...
def card(an, now):
    ...
    # Тотал-картата казва СА́МА какво е. Без този ред читателят вижда втора
    # карта за същия мач и не разбира защо ботът се повтаря.
    if str(an.get("vid_karta") or "") == "total":
        lines.append("<i>втори пазар от същия мач — сборът, не победителят</i>")
    if KOEF_VKL:
    ...
    # 🔴 ИСТОРИЯТА НЕ СЕ ЛЕПИ НА ЧУЖД ПАЗАР (19.08.2026). „Футболът досега:
    # 21 от 42" се брои върху карти 1/Х/2. Сложен под карта за тотал, същият
    # ред би се прочел като история НА ТОТАЛИТЕ — а такава още нямаме нито
    # една отсъдена. По-добре празно, отколкото чужда сметка.
    rec = ("" if str(an.get("vid_karta") or "") == "total"
...
def selftest():
    ...
    try:
        ...
        _txt = card(_tk, now)
        check("картата казва, че е втори пазар", "втори пазар" in _txt)
